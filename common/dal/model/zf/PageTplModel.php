 ...
 */
class PageTplModel extends AbstractZaFulModel
{
    public $query;
    ...
    public function getTplList($params)
    {
        ...
        if (!empty($params['pipeline'])) {
            $this->query->andWhere(['pipeline' => $params['pipeline']]);
        }
        switch ($params['type']) {
            case 1://公有模板
        ...
        $list = $this->query
            ->select('id,pid,name,pic,site_code,pipeline,place,platform_type,lang,tpl_type,is_default,create_user,create_time,update_time')
