 ...
 * @property int    $tpl_id
 */
class PageUiComponentDataModel extends AbstractZaFulModel
{
    //是否公共数据|1-是
    const IS_PUBLIC = 1;
    ...
    //需要copy的字段名
    const COPY_FILEDS = ['goodsSKU', 'goodsSKUSort', 'goodsSKUList', 'textTitle', 'textCode', 'couponId', 'startText',
        'getCouponText', 'couponTitle', 'goodsDataSource', 'cateId', 'goodsNum', 'goodsIds', 'goodsID', 'goodsIDnum',
        'productsNum', 'goodsID', 'goodsSKUTab', 'imgItem', 'community', 'communityLinkList', 'communityRgDefaultList',
        'communitySkuList'];
    ...
    public static function copySku(array $uiEqualList, array $langEqual)
    {
        foreach ($uiEqualList as $ui) {
             ...
             */
            $fromSkus = static::find()->where([
                'component_id' => $ui['fromId'],
                'lang' => $langEqual['fromLang'],
                'key' => static::COPY_FILEDS,
            ])->andWhere('tpl_id = ' . $ui['fromTplId'] . ' OR is_public = ' . static::IS_PUBLIC)->asArray()->all();
            if (!$fromSkus) {
                continue;
            }

            foreach ($fromSkus as $fromSku) {
                $toSku = static::find()->where([
                    'component_id' => $ui['toId'],
                    'lang' => $langEqual['toLang'],
                    'key' => $fromSku['key'],
                ])->andWhere('tpl_id = ' . $ui['toTplId'] . ' OR is_public = ' . static::IS_PUBLIC)->one();
                if (!$toSku) {
                    $toSku = new static();
                    $toSku->component_id = $ui['toId'];
                    $toSku->lang = $langEqual['toLang'];
                    $toSku->key = $fromSku['key'];
