<script type="text/javascript">
    ...
    var GESHOP_IS_HOME_B = "<?php echo $isHomeB;?>";
    var GESHOP_HAS_ALL_HOME_PERMISSIONS = "<?php echo $hasAllHomePermissions;?>";
    var GESHOP_HAS_PERMISSION_PIPELINES = <?php echo json_encode($hasPermissionPipelines);?>;
...
?>
    ...
    <link rel="stylesheet" href="<?= app()->url->assets->css('resources/stylesheets/design.index.css'); ?>">

    <style>
        /* <!-- rg字体 --> */
        @font-face {
            font-family: Rubik-Regular;
            src: url("<?= app()->url->assets->css('temp/skin3/dist/font/Rubik-Regular/Rubik-Regular.woff'); ?>");
        }
        @font-face {
            font-family: Rubik-Medium;
            src: url("<?= app()->url->assets->css('temp/skin3/dist/font/Rubik-Medium/Rubik-Medium.woff'); ?>");
        ...
        #ab_test_temp .content{
            margin-left: 40px;
            margin-right: 40px;
        }
        #ab_test_temp .layui-input-block{
            margin-top:20px;
        }
        #ab_test_temp input{
        ...
        #ab_test_temp label:first-child{
            margin-right:100px;
        }
        ...
        .design-control-lang-form > * {
            ...
            vertical-align: bottom;
                ...
                .geshop-right-options{
                    float:right;
                }
                .geshop-right-options .layui-form-switch{
                    margin-right:15px;
                    margin-top:0;
                }
    </style>
    ...
    <header class="site-header-container">
        <div class="left">
            <h1>
                <a href="/">
                    <img src="/resources/images/geshop-logo.png" style="display:block;height:30px;" alt="geshop logo" />
                </a>
            </h1>
        ...
        <div class="right">
            <form class="layui-form" action="">
                ...
                <button type="button" id="preview_page" class="btn" data-href="<?= $preview_url; ?>">本地预览</button>
                <?php if ($pageInfo['status'] != '1' ): ?>
                <button type="button" id="preview_pageUrl" class="btn" data-href="<?= $pageUrl; ?>">线上预览</button>
                <?php endif; ?>
                <?php if ($pageInfo['status'] == '2' ): ?>
                <button type="button" id="view_page" class="btn">访问</button>
                <?php endif; ?>
                <button type="button" id="release_page_AB" class="btn">发布</button>
                <ul class="layui-nav" style=" width: 100px; padding: 0; display: inline-block; background-color: #1e9fff; z-index: 201; border-radius:0;">
                    <li class="layui-nav-item" style="display: block; width: 100%; line-height: 64px;">
                    <a id="generate_page_AB" style="padding:0 10px;color:#FFF;text-align: center;" href="javascript:;">设为首页</a>
                    </li>
                </ul>
    ...
    <main class="site-main-design site-main-home">
            ...
            <div class="sidebar-container">
                ...
                <div class="sidebar-content hide" style="width:320px">
                    ...
                    <div class="sidebar-list-container">
                        ...
                        <ul class="sidebar-component-container sidebar-component-container-big">
                            <?php if (!empty($data['ui'])) { ?>
                            <?php foreach ((array)$data['ui'] as $value) { ?>
                            <li class="design-model-item component-drag" draggable="true" data-key="<?= $value->component_key ?>" data-name="<?= $value->name ?>"
                                <?php if ($value->component_key == 'U000076' || $value->component_key == 'U000077'): ?>data-unique="true"<?php endif; ?>
                            >
...
<input type="text" name="templateName" class="layui-input" placeholder="组件筛选">
        ...
        <div class="design-right">
            <div class="el-row design-control">
                ...
                <div>
                    ...
                    <div class="el-col el-col-12">
                        <div class="geshop-right-options">
                            <?php if ($isRollbackLock == '1'): ?>
                                <div class="layui-form" style="display:inline-block;">
                                    <input type="checkbox" name="page_lock" id="page_lock" lay-skin="switch" lay-text="解锁|锁定" lay-filter="page_lock" checked>
                                </div>
                            <?php endif; ?>
                            <?php if ($isRollbackLock != '1'): ?>
                                <div class="layui-form layui-hide" style="display:inline-block;">
                                    <input type="checkbox" name="page_lock" id="page_lock" lay-skin="switch" lay-text="解锁|锁定" lay-filter="page_lock" disabled>
                                </div>
                            <?php endif; ?>
            ...
            <div id="design_view" class="design-view layout-drop" style="margin:120px auto 20px;width:401px;">
                <!-- index start -->
                <?= $pageHtml; ?>
            ...
            <div class="design-form design-form-component">
                <form id="component_form" class="layui-form">
                    <!-- form start -->
    ...
    <!-- 是否锁定 -->
    <input id="isRollbackLock" type="hidden" value="<?= $isRollbackLock ?>">
    <!-- 是否新渠道 -->
    <input id="isNewPipeline" type="hidden" value="<?= $isNewPipeline ?>">
    ...
    <script id="ab_test" type="text/template">
        <div class="gs-required">
            <form id="ab_test_pipeline_wraper" class="layui-form" action="">
                <div class="layui-form-item>
                    <div class="layui-input-block layui-form-title" style="margin-left:24px">
                        <span class="gs-form-label">渠道</span>
                        <p style="margin-left:15px;"><input type="checkbox" name="release_check_all" lay-skin="primary" lay-filter='release_check_all' title="全选" value="1"></p>
                    </div
                </div>

                <div class="layui-form-item pipeline_list">
                    <div class="layui-input-block">
                    <?php foreach ($hasPermissionPipelines as $item) { ?>
                        <input type="checkbox" name="<?= $item['pipeline']; ?>" title="<?= $item['pipeline_name']; ?>" lay-skin="primary" class='pipeClass' lay-filter='release_check_one'>
                    <?php } ?>
                    </div>
                </div>
            </form>
            <div id="ab_test_temp">
                <div class="layui-input-block gs-form-label layui-form-title" style="margin-left:24px">首页类型</div>
                <div class="content">
                    <div>
            ...
            <blockquote class="layui-elem-quote" style="margin: 15px 24px;font-size:12px;border-color: #1E9FFF;">温馨提示：发布首页后请点击“<span style="color:red;">线上预览</span>”检查页面内容是否正常,
                                                                                                       请确认“线上预览”页面无误后点击“<span style="color:red;">设为首页</span>”即可更新线上页面哦~</blockquote>
        </div>
    </script>
    ...
    <?php
$this->registerJsFile(
    app()->url->assets->js('designHome.js'),
...
$this->registerJsFile(
    app()->url->assets->js('resources/javascripts/design.index.site.js'),
    ['position' => \yii\web\View::POS_END]
);

$this->registerJsFile(
    app()->url->assets->js('resources/javascripts/design.common.js'),
    ['position' => \yii\web\View::POS_END]
);
