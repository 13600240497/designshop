<script type="text/javascript">
    ...
    var GESHOP_HOME_PAGE_STATUS = "<?php echo $pageStatus;?>";
...
?>
    ...
    <header class="site-header-container">
        ...
        <div class="right">
            <form class="layui-form" action="">
                ...
                <ul class="layui-nav" style="width: 120px; padding: 0; display: inline-block; background-color: #1e9fff; z-index: 201; border-radius:0;">
                    <li class="layui-nav-item" style="display: block; width: 100%; line-height: 64px;">
                    <a id="generate_and_set" style="padding:0 10px;color:#FFF;" href="javascript:;">发布并设为首页</a>
                    </li>
                </ul>
            </form>
        </div>
                ...
                <div>
                    <div class="el-col el-col-12">
                        <div class="design-control-lang">
                            <?php if (count($homelangList) > 4) { ?>
                                <form class="layui-form" action="">
                                    <select name="" lay-filter="selectLang">
                                        <?php foreach ($homelangList as $item) { ?>
                                        <option value="<?= \yii\helpers\Url::current(['lang' => $item['key']]); ?>" <?= $lang !== $item['key'] ? '' : 'selected'; ?>><?= $item['name']['name']; ?></option>
                                        <?php } ?>
                                    </select>
                                </form>
                            <?php } else {
                                foreach ($homelangList as $item) {
                            ?>
    ...
    <script id="ab_test" type="text/template">
        <form id="ab_test_lang_wraper" class="layui-form" action="">
            <div class="layui-form-item">
                <div class="layui-input-block">
                <?php foreach ($homelangList as $item) { ?>
                    <input type="checkbox" name="<?= $item['key']; ?>" title="<?= $item['name']['name']; ?>" lay-skin="primary">
                <?php } ?>
                </div>
            </div>
        </form>
...
$this->registerJsFile(
    app()->url->assets->js('resources/javascripts/design.index.js'),
    ['position' => \yii\web\View::POS_END]
);
