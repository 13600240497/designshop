...
class PageComponent extends CommonPageComponent
{
    ...
    public function lists(array $params)
    {
        ...
        $query = PageModel::find()->alias('h')
            ...
            ->where(['h.is_delete' => 0, 'h.activity_id' => 0, 'h.site_code' => $siteCode])
            ->andWhere(
    ...
    public function multiPlatformAdd(array $params)
    {
        $validPlatformParams = [];
        //首页活动支持PC和WAP端
        $supportPlatforms = [SitePlatform::PLATFORM_CODE_PC, SitePlatform::PLATFORM_CODE_WAP];
        foreach ($supportPlatforms as $platformCode) {
            ...
            // 检查参数合法性
            if (empty($platformParams['site_code'])
                || !SitePlatform::isCurrentSiteGroupPlatformSite($platformParams['site_code'])
            ) {
            ...
            $supportLanguageKeys = SitePlatform::getSiteHomePageSupportLanguageKeys($platformParams['site_code']);
