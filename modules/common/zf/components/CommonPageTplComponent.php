...
use app\modules\common\zf\models\{
     PageLayoutDataModel, PageModel, PageLayoutModel, PageUiModel, PageTplModel, PageUiDataModel
};
    ...
    */
    ...
    public function getList($params)
    {
        ...
        if (!empty($params['site_code'])) {
	        ...
	        $params['range'] = !empty($params['is_native']) ? 4 : 0;
        }
    ...
    public function initPageTpl($pageId, $tplId = 0, $lang)
    {
        ...
        $this->copyPage($pageId, $tplInfo, $lang, true);
     ...
     * @param string $copyLang 需复制语言(为空表示页面所有语言复制)
     * @param bool $copySku 是否包含复制SKU
     */
    public function copyPage($pageId, $tplInfo, $copyLang = '', $copySku = false)
    {
        ...
        foreach ($pages['pageLanguages'] as $lang) {
            ...
            foreach ($layout as $key => $item) {
                ...
                $layoutDataModel->style_data = $layoutData[$key]['style_data'] ?? '';
            ...
            !empty($layout) && $this->handleUi($layoutArr, $ui, $uiData, $lang['lang'], $copySku);
        }
    }
     ...
     * @param bool $copySku 是否包含复制SKU
     */
    private function handleUi($layoutArr, $ui, $uiData, $languages, $copySku)
    {
        //$newMenu 新导航需导航组件数组
        ...
        foreach ($ui as $key => $item) {//单个布局组件
            foreach ($item as $k => $val) {//布局组件单列
                ...
                foreach ($val as $uiItem) {
                    ...
                    $uiModel->async_data_format = $uiItem['async_data_format'] ?? '';
                    $uiModel->save();
                    $nextId = $uiModel->id;
                    //当前旧组件ID在需要导航数据内
