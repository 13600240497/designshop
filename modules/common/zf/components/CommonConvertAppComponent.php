...
class CommonConvertAppComponent extends Component
{
    ...
    public function getCreatorAppLists($activityType=SiteConstants::ACTIVITY_TYPE_SPECIAL)
    {
        ...
        $activityList = ActivityModel::find()->alias('a')
            ...
            ->andWhere(['g.platform_list' => SitePlatform::PLATFORM_CODE_APP]) //过滤只有APP端口的活动
     ...
     */
    private function mergeActivityPageList(array &$activityList, array $pageList, string $siteGroupCode)
    {
        $pageArr = [];
        $pipelineList = [];
        ...
        try {
            ...
            foreach ($targetPageModelList as $targetPipeline => $targetPageModel) {
                ...
                if (self::CONVERT_MODEL_APPEND === (int)$params['model']) {
                    //追加到目标页面内容后面
                    if (true !== ($res = $this->appendConvertApp($targetPageModel->id, $componentData, $params['target'][$targetPipeline]))) {
                        throw new Exception($res);
                    }
                } elseif (true !== ($res = $this->coverConvertApp($targetPageModel->id, $componentData, $params['target'][$targetPipeline]))) {
                    //会直接覆盖目标页面内容
                    throw new Exception($res);
                }
     ...
     * @throws \Throwable
     */
    public function appendConvertApp(int $targetId, array $componentData, array $targetInfo)
    {
        ...
        if (!empty($layoutComponent['data'])) {
            ...
            foreach ($targetInfo['lang_list'] as $targetLang) {
                ...
                //将UI组件和组件数据的语言转换为目标语言
                foreach ($layout as $value) {
                    if (!empty($uiComponent['data'][ $value['id'] ]) && is_array($uiComponent['data'][ $value['id'] ])) {
                        foreach ($uiComponent['data'][ $value['id'] ] as &$_uiComponentItem) {
                            $_uiComponentItem['lang'] = $targetLang;
                        }
                    }
                }
                if (!empty($uiData['data']) && is_array($uiData['data'])) {
                    foreach ($uiData['data'] as &$_uiDataItemList) {
                        foreach ($_uiDataItemList as &$_uiDataItem) {
                            $_uiDataItem['lang'] = $targetLang;
                        }
                    }
                }
     ...
     * @throws \Throwable
     */
    public function coverConvertApp(int $targetId, array $componentData, array $targetInfo)
    {
        ...
        if (!empty($layoutComponent['data'])) {
            foreach ($targetInfo['lang_list'] as $targetLang) {
                foreach ($layoutComponent['data'] as $layout) {
                    ...
                    foreach ($layout as $value) {
                        $componentDataArr = [$targetLang, $targetId, $value['component_key'], $nextId];
                        PageLayoutModel::insertAll($componentColumns, [$componentDataArr]);
                        $nextId = PageLayoutModel::getDb()->getLastInsertID();

                        //将原语言转换为目标语言
                        if (!empty($layoutData['data'][ $value['id'] ]) && is_array($layoutData['data'][ $value['id'] ])) {
                            foreach ($layoutData['data'][$value['id']] as &$_layoutDataItem) {
                                $_layoutDataItem['lang'] = $targetLang;
                            }
                        }
                        ...
                        //将UI组件和组件数据的语言转换为目标语言
                        if (!empty($uiComponent['data'][$value['id']]) && is_array($uiComponent['data'][$value['id']])) {
                            foreach ($uiComponent['data'][$value['id']] as &$_uiComponentDataItem) {
                                $_uiComponentDataItem['lang'] = $targetLang;
                            }
                        }
                        if (!empty($uiData['data']) && is_array($uiData['data'])) {
                            foreach ($uiData['data'] as &$_uiDataItemList) {
