...
class CommonPageLayoutDesignComponent extends CommonPageDesignComponent
{
    ...
    public function copyLayoutComponent($data)
    {
        ...
        try {
            for ($i = 0; $i < $num; $i++) {
                ...
                $layout->pipeline = $this->pageInfo->pipeline;
    ...
    public function deleteLayoutComponent($data)
    {
        ...
        //通过layout_id获取ui_id
        //$ui_ids = PageUiModel::getUiIdsByLayoutId($data['id']);
        ...
        foreach ($ui_ids as $ui_id => $ui_lang){
            ...
            //取消三端数据绑定
            $ui->cancelUiComponentBindRelation($ui_id, $ui_lang);
            $sync_data['del_info'][] = [
    ...
    public function getLayoutForm($layoutId)
    {
        ...
        if ($layoutDataModel) {
            ...
            $layout->customData = CommonLayout::getCustomData($customData, $layoutDataModel->style_data);
        }
    ...
    public function saveLayoutForm($data, $runValidation = true)
    {
        ...
        $styleData = [
            ...
            'background_repeat_position' => $data['background_repeat_position'] ?? '',
            'background_layout_color' => $data['background_layout_color'] ?? '',
        ];

        /** @var \app\modules\common\zf\models\PageLayoutDataModel $layoutDataModel */
        ...
        if (!$layoutDataModel) {
            ...
            $layoutDataModel->style_data = json_encode([]);
        }
        ...
        $layoutDataModel->style_data = json_encode($styleData);
        ...
        if ($layoutDataModel) {
            ...
            $layout->customData = array_merge($customData, $styleData);;
        }
