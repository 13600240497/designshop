...
trait CommonPageParseTrait
{
	...
	{
		list($pageId, $lang, $siteCode, $pipeline, $version, $needBreakParse, $useCache, $updateGoods, $updateUiGoods) = $params;
		...
		if ($useCache) {
			...
			if ($updateGoods && $useCache) {//只需要刷新页面SKU并且发布缓存表记录不为空才执行
				...
				if (!empty($uilist) && is_array($uilist)) {
					(new ExplainComponent())->promiseSetGoodsList($uilist, $siteCode, $pipeline, $lang);
				}

				$data['html'] = $this->getPageHtml(json_decode($publishCacheModel['layout'], true), $uilist, false);
		...
		if (!$useCache) {
			...
			if (!empty($uiListByLayoutPosition) && is_array($uiListByLayoutPosition)) {
				//按照站点,渠道,语言的维度把页面商品数据加入到临时缓存
				(new ExplainComponent())->promiseSetGoodsList($uiListByLayoutPosition, $siteCode, $pipeline, $lang);
			}

			$data['html'] = $this->getPageHtml($orderedLayouts, $uiListByLayoutPosition, false);
	 ...
	 * @throws \yii\db\Exception
	 */
	public function parseNativePageInArray (array $params)
	{
		list($pageId, $lang, $siteCode, $pipeline, $version, $needBreakParse, $useCache, $updateGoods, $updateUiGoods) = $params;
		$this->needBreakParse = $needBreakParse ?? false;
		$data = [
		...
		if ($useCache) {
			...
			if ($updateGoods) {
				...
				if ($updateUiGoods) {
					$uilist = NativePageUiComponentModel::getComponentsData($pageId, $lang, $publishCacheModel['layout']);//获取组件最新产品
					$data['uilist'] = is_array($uilist) ? $uilist : json_encode($uilist);
				} else {
					$uilist = is_array($publishCacheModel['uilist']) ? $publishCacheModel['uilist'] : json_decode($publishCacheModel['uilist'], true);
					$data['uilist'] = $publishCacheModel['uilist'];
				}

				if (!empty($uilist) && is_array($uilist)) {
					$explainComponent->promiseSetGoodsList($uilist, $siteCode, $pipeline, $lang, true);
				}

				$publishCacheModel['layout'] = is_array($publishCacheModel['layout']) ? $publishCacheModel['layout'] : json_decode($publishCacheModel['layout'], true);
				$data['html'] = $this->getNativePageHtml($pageId, $siteCode, $pipeline, $lang, $publishCacheModel['layout'], $uilist);
				$data['layout'] = $publishCacheModel['layout'];
				$data['version'] = $version;
				$data['page_id'] = $pageId;
				$data['lang'] = $lang;
		...
		if (!$useCache) {
			//获取页面单纯的html
			$layout = NativePageLayoutModel::getComponentsSort($pageId, $lang);
			$uilist = NativePageUiComponentModel::getComponentsData($pageId, $lang, $layout);

			if (!empty($uilist) && is_array($uilist)) {
				$explainComponent->promiseSetGoodsList($uilist, $siteCode, $pipeline, $lang, true);
			}

			$data['html'] = $this->getNativePageHtml($pageId, $siteCode, $pipeline, $lang, $layout, $uilist);
			$data['layout'] = is_array($layout) ? $layout : json_encode($layout); //排好序的组件列表
			$data['uilist'] = is_array($uilist) ? $uilist : json_encode($uilist); //UI组件列表数据
	 ...
	 * @throws \yii\base\ViewNotFoundException
	 */
	public function getHeadAndFooterByPageId ($pageId, $lang, array $componentStatic, string $pipeline)
	{
		$data = '';
		...
		if ($pageData = PageModel::getPageInfo($pageId, $lang, true)) {
			...
			$data = $explainTpl->getTplContent($pageConfig, $pipeline);
		}
	 ...
	 * @throws \yii\base\ViewNotFoundException
	 */
	public function getHeadAndFooterByTplId ($tplId, $lang, $pipeline)
	{
		$data = '';
	...
	public function getPageLayoutByPageId ($pageId, $lang)
	{
		...
		if (empty($data[$key])) {
			...
			$data[$key] = ArrayHelper::toArray($layoutList);
		}
	...
	private function activityPageLayoutByPageId ($pageId, $lang)
	{
		return PageLayoutModel::find()->alias('l')
			->select('l.*, d.data, d.custom_css, d.background_color, d.background_img, d.style_data, a.site_code, p.activity_id, p.pipeline')
			->leftJoin(
	...
	{
		return PageLayoutModel::find()->alias('l')
			->select('l.*, d.data, d.custom_css, d.background_color, d.background_img, d.style_data, p.site_code, p.activity_id, p.pipeline')
			->leftJoin(
				PageLayoutDataModel::tableName() . ' as d',
				'l.id = d.component_id'
			)->leftJoin(
	...
	{
		static $data = [];
		$key = md5((is_array($layoutIds) ? implode(':', $layoutIds) : $layoutIds) . ":{$lang}");
		if (empty($data[$key])) {
	 ...
	 * 解析Layout组件
	 *
	 * @param \app\modules\common\zf\components\CommonLayout $layout
	...
	private function parseLayout ($layout, $layoutArr, $uiData)
	{
		...
		$layout->customData = CommonLayout::getCustomData($customData, $layoutArr['style_data'] ?? '');
	...
	private function getPageHtml ($orderedLayouts, $uiListByLayoutPosition, $isPageTplExplain = false)
	{
		...
		if (!empty($orderedLayouts)) {
			...
			$ui->pipeline = $orderedLayouts[0]['pipeline'] ?? '';
			...
			$layout->pipeline = $orderedLayouts[0]['pipeline'] ?? '';
	 ...
	 * @return string
	 */
	private function getNativePageHtml ($pageId, $siteCode, $pipeline, $lang, $components, $componentData)
	{
		$pageData = $this->getNativeUiData($siteCode, $pipeline, $lang, $components, $componentData);
		$languages = (new LanguageDataComponent())->getLanguageDataForLang($lang);
		$nativeId = PageModel::getNativeAppPageId($pageId, $siteCode, $pipeline);

		// Growing IO 组件埋点数据生成
		$growingIoUiInfo = \app\base\GrowingIO::generateNativeUiList($siteCode, $pageId, $lang, $componentData);
		...
		$uiContextData = [
			...
			'growingIoUiInfo' => $growingIoUiInfo
        ];
		return app()->view->renderFile(APP_PATH . '/modules/activity/zf/views/release.php', $uiContextData);
	}
	 ...
	 * @return array
	 */
	public function getNativeUiData ($siteCode, $pipeline, $lang, $components, $componentsData)
	{
		//自定义布局组件编码
		...
		if (!empty($componentsData) && is_array($componentsData)) {
			...
			foreach ($componentsData as &$component) {
                ...
                // 商品详情信息通过异步接口获取，这里注释掉，商品数据不打到页面上了
...
//				}
	 ...
	 * @param array                                      $uiListByLayoutPosition 按LayoutId和position分好组的UI组件列表
	 * @param int                                        $layoutId               layout组件ID
	 * @param \app\modules\common\zf\components\CommonUi $ui                     ui组件
	 *
	 * @return array
	 * @throws \ego\base\JsonResponseException
	 * @throws \yii\base\ViewNotFoundException
	...
	public function getHeadExtraCss ($pageId, $lang, $cssVersion, $siteCode, $activityPageType, $isPublish = false)
	{
		...
		if ($pageId) {
			...
			if ($pageModel = PageModel::findOne($pageId)) {
				$pipeline = $pageModel->pipeline;

				if ($isPublish) {
					// 国家站点自动跳转
					if (($pageModel->activity_id > 0) && (1 === (int)$pageModel->is_redirect_country)
						&& !SitePlatform::isAppPlatform($siteCode)
					) {
						$isRedirectCountryPage = 1;
						$countryPageUrlsJs = $this->getCountryPageUrlsJs(
							$isRedirectCountryPage, $pipeline, $siteCode, $pageModel->group_id
						);
					}
				}
			}
		...
		$isDirectUseApiFallbackData = (int)SysConfigUtils::get('sys.ui.zf.direct_use_api_fallback_data', 0);
		$jsIsDirectUseFallback = (($isUiApiFallbackEnabled === 1) && ($isDirectUseApiFallbackData === 1)) ? 1 : 0;

		$css = /** @lang html */
			'<script type="text/javascript">'
			. ' var HTTPS_REDIRECT_LINK = "' . $redirectUrl . '";'
			. ' var GESHOP_LANG = "' . $lang . '";'
			. ' var GESHOP_PAGE_TYPE = "' . $jsPageType . '";'
			. ' var GESHOP_ENV_TYPE = "' . $envType . '";'
			. ' var GESHOP_PID = "' . $pageId . '";'
			. ' var GESHOP_PUBLISHED_TIME = "' . time() . '";'
			. ' var GESHOP_PIPELINE = "' . $pipeline . '";'
			. ' var GESHOP_SITECODE = "' . $siteCode . '";'
			. ' var GESHOP_PLATFORM = "' . $platform . '";'
			. ' var GESHOP_MULTI_TIME_STYLE = "' . $multiTimeStyle . '";'
			...
			. ' var GESHOP_IS_REDIRECT_COUNTRY_PAGE = ' . $isRedirectCountryPage . ';'
		...
		$cssFilenameSuffix = ($activityPageType === SiteConstants::ACTIVITY_PAGE_TYPE_HOME) ? 'home' : 'activity';
		if (explode('-', $siteCode)[0] == 'dl') {
			...
			$cssGridHtml = sprintf($cssGridFormat, $staticDomain, 'default', $cssVersion);
		} else {
			$cssGridFormat = '<link rel="stylesheet" href="%s/resources/sitesPublic/%s/css/geshop-grid_%s.css?version=%s">';
			$cssGridHtml = sprintf($cssGridFormat, $staticDomain, 'default', $cssFilenameSuffix, $cssVersion);
		}
		$cssCommonFormat = '<link rel="stylesheet" href="%s/frontend/geshop/stylesheets/geshop-components-default-%s.css?version=%s">';
		$cssCommonHtml = sprintf($cssCommonFormat, $staticDomain, $platformCssSuffix, $cssVersion);

		return $css . $cssGridHtml . $cssCommonHtml;
	}

	/**
	 * 国家站活动页面URL，根据用户访问国家跳转到对应的渠道活动页面，如果对应的渠道没有页面则跳到对应的渠道首页
	 *
	 * @param int    $isRedirectCountryPage
	 * @param string $siteCode    站点简码
	 * @param string $pipeline    当前渠道
	 * @param int    $pageGroupId 子页面分组ID
	 *
	 * @return string
	 */
	private function getCountryPageUrlsJs ($isRedirectCountryPage, $pipeline, $siteCode, $pageGroupId)
	{
		if ($isRedirectCountryPage) {
			$countryPageUrls = $this->getCountryPageUrls($siteCode, $pageGroupId);
			isset($countryPageUrls[$pipeline]) && $countryPageUrls[$pipeline] = '';
			return sprintf('var GESHOP_COUNTRY_PAGE_URLS=%s;', json_encode($countryPageUrls, JSON_UNESCAPED_UNICODE));
		}
		return '';
	}

	/**
	 * 获取当前发布页面的其他渠道活动页面URL
	 *
	 * @param string $siteCode    站点简码
	 * @param int    $pageGroupId 子页面分组ID
	 *
	 * @return array
	 */
	private function getCountryPageUrls ($siteCode, $pageGroupId)
	{
		static $countryPageUrls = [];
		if (isset($countryPageUrls[$pageGroupId])) {
			return $countryPageUrls[$pageGroupId];
		}

		$pageInfoMapping = PageModel::find()
			->select('id, pipeline, default_lang')
			->where(['group_id' => $pageGroupId])
			->indexBy('pipeline')
			->asArray()
			->all();
		if (empty($pageInfoMapping)) {
			return [];
		}

		$pageIds = array_column($pageInfoMapping, 'id');
		$languageInfoMapping = PageLanguageModel::find()->select('page_id, lang, url_name')
			->where(['page_id' => $pageIds])
			->indexBy(function ($row) {
				return $row['page_id'] . '_' . $row['lang'];
			})->asArray()->all();

		$pageUrls = [];
		$pipelineList = PipelineUtils::getConfigSpecialSupportPipelineList($siteCode);
		foreach ($pipelineList as $pipelineCode => $langList) {
			$pageUrl = '';
			if (isset($pageInfoMapping[$pipelineCode])) {
				$pageId = $pageInfoMapping[$pipelineCode]['id'];
				$langCode = $pageInfoMapping[$pipelineCode]['default_lang'];
				$key = $pageId . '_' . $langCode;
				if (isset($languageInfoMapping[$key])) {
					$pageUrl = $langList[$langCode] ?? '';
					$pageUrl .= '/' . $languageInfoMapping[$key]['url_name'] . '.html';
				}
			}

			if (empty($pageUrl)) {
				$langCode = app()->params['site']['zf']['pipeline_default_lang'][$pipelineCode];
				$pageUrl = $langList[$langCode] ?? '';
				$pageUrl = str_replace('/promotion', '', $pageUrl);
			}

			$pageUrls[$pipelineCode] = $pageUrl;
		}

		$countryPageUrls[$pageGroupId] = $pageUrls;
		return $countryPageUrls[$pageGroupId];
	}
	...
	public function getHeadExtraJs ($version, $lang, $siteCode, $activityPageType, $isPublish = false)
	{
		...
		$jsHtml = sprintf('<script src="%s/resources/javascripts/library/LAB.js?version=%s"></script>', $staticDomain, $version);
	...
	public function getAvailableNavigation ($uiComponentId, $lang)
	{
		...
		if (!isset($data[$key])) {
			...
			$data[$key] = $pageLayoutInfo ? $this->getNeedNavigateUiTitle($pageLayoutInfo->page_id, $lang) : [];
		}
	 ...
	 * 格式化页面模板数据
	 *
	 * @param \app\modules\common\zf\models\PageTplModel $item
	 *
	 * @return array
	 */
	private function formatData ($item)
	{
		...
		if (!empty($layout)) {
			...
			foreach ($layout as $k => $v) {
				...
				$layout[$k]['pipeline'] = $item->pipeline;
			}
		}
