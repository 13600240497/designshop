...
use app\modules\common\gb\models\{
    ActivityModel, PageLayoutDataModel, PageModel, PageLayoutModel, PageUiModel
};
...
class CommonPageLayoutDesignComponent extends CommonPageDesignComponent
{
    ...
    public function getLayoutForm($layoutId)
    {
        ...
        $layout = new CommonLayout();//布局对象
        if ($layoutDataModel) {
            $styleData = !empty($layoutDataModel->data) ? json_decode($layoutDataModel->data, true) : [];
    ...
    public function saveLayoutForm($data, $runValidation = true)
    {
        ...
        $customData = !empty($data['data']) ? json_decode($data['data']) : [];
        ...
        $layoutData = !empty($layoutDataModel->data) ? json_decode($layoutDataModel->data, true) : [];
        if (!empty($layoutData['custom_data'])) {
            unset($layoutData['custom_data']);
        }
        $layoutDataModel->data = json_encode(
            [
                'layout_data' => isset($layoutData['layout_data']) ? $layoutData['layout_data'] :$layoutData,
        ...
        if ($layoutDataModel) {
            ...
            //设置自定义CSS和背景图等属性
            $layout->customData = [
                'custom_css'        => $layoutDataModel->custom_css,
                'background_color' => $layoutDataModel->background_color,
                'background_img' => $layoutDataModel->background_img
            ];*/
            $styleData = !empty($layoutDataModel->data) ? json_decode($layoutDataModel->data, true) : [];
