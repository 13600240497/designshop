...
use app\modules\common\gb\models\{
    PageLanguageModel, PageLayoutModel, PageLayoutDataModel, PageUiModel, PageUiDataModel,
...
class CommonPageUiComponentDataComponent extends Component
{
    ...
    private static function getConfigById($id, $field)
    {
        ...
        $item = $item ? ArrayHelper::toArray($item) : [];

        if (empty($item)) {
            return false;
        }

        $path = app()->basePath . '/files/parts/ui/' . $item['component_key'] . '/' . $item['name_en'];
    ...
    private function updateUiData($id, $tplId, $lang, &$data)
    {
        ...
        $list = PageUiComponentDataModel::find()->where($where)->asArray()->all();
        if (empty($list)) {
            return true;
        }
     ...
     * @return array
     */
    private function formatData($default, $data)
    {
    ...
    {
        $list = PageLayoutModel::find()->where(['page_id' => $pageId])->asArray()->all();
        //无布局组件
        if (empty($list)) {
            return true;
        }
