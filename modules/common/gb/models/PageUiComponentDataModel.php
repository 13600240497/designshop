 ...
 * @property int    $tpl_id
 */
class PageUiComponentDataModel extends BaseModel
{
    //是否公共数据|1-是
    const IS_PUBLIC = 1;
    ...
    public static function copySku(array $uiEqualList, array $langEqual)
    {
        foreach ($uiEqualList as $ui) {
             ...
             */
            $fromSku = static::find()->where([
                'component_id' => $ui['fromId'],
                'lang' => $langEqual['fromLang'],
                'key' => static::KEY_SKU,
            ])->andWhere('tpl_id = ' . $ui['fromTplId'] . ' OR is_public = ' . static::IS_PUBLIC)->one();
            if (!$fromSku) {
                continue;
            }
