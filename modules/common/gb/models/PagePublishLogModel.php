...
class PagePublishLogModel extends BaseModel
{
    ...
    public static function getPageNewestPublishLog(int $pageId, int $status)
    {
        ...
        if (!empty($version['version'])) {
            $data = $query->select('l.*, pl.page_url,pl.pipeline')
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public static function getPageNewestPushLog(int $pageId, array $version)
    {
        return static::find()->where([
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public static function getPageNewestPublishLogList(string $groupId, int $status)
    {
        $actionType = ($status === PageModel::PAGE_STATUS_HAS_ONLINE || $status === PageModel::PAGE_STATUS_HAS_ONLINE_UPDATE)
            ? self::ACTION_TYPE_ONLINE : self::ACTION_TYPE_OFFLINE;
        ...
        $versionResult = $query->select('l.version')
            ->leftJoin(PageLanguageModel::tableName() . ' as pl', 'l.page_id = pl.page_id AND l.lang = pl.lang')
            ->andWhere(['pl.group_id' => $groupId])
            ->groupBy('l.version')
            ->orderBy('l.version desc')
            ->asArray()
            ->all();
        ...
        if (!empty($versionResult)) {
            $version = array_column($versionResult, 'version');
            $data = $query->select('l.*, pl.page_url,pl.pipeline')
                ->andWhere(['in', 'l.version', $version])
                //->groupBy('l.page_id,l.lang, l.file_type')
                ->groupBy('l.lang, l.version, l.file_type')
                ->asArray()
                ->all();
        }
     ...
     * @return array|\yii\db\ActiveRecord[]
     */
    public static function getPageNewestPushLogList(array $version)
    {
        return  static::find()->alias('l')
            ->where(['l.log_type' => self::LOG_TYPE_PUBLISH])
            ->leftJoin(PageLanguageModel::tableName() . ' as pl', 'l.page_id = pl.page_id AND l.lang = pl.lang')
            ->andWhere(['in', 'l.version', array_unique($version)])
            ->asArray()
            ->all();
    }
}
