...
use app\modules\activity\gb\components\PageComponent;
use app\modules\common\gb\models\{
    ActivityModel, PageModel, PageLanguageModel, PagePublishCacheModel, PageLayoutModel, PageLayoutDataModel,
    PageUiModel, PageTplModel, PageUiComponentDataModel, PageGroupModel
};

use app\modules\common\gb\components\{
...
trait CommonPageParseTrait
{
    ...
    public function parsePage($pageId, $lang, $needBreakParse = false, $useCache = false, $build = true)
    {
        ...
        return !$build ? $data :
            /** @lang html */
            $data['html'] . '<style type="text/css">' . $data['css'] . '</style>'
    ...
    public function getHeadAndFooterByPageId($pageId, $lang, array $componentStatic = [])
    {
        ...
        if ($pageData = PageModel::getPageInfo($pageId, $lang, true)) {
            ...
            $pageConfig->pipeline = $pageData['pipeline'];
            ...
            $pageConfig->special_id = PageGroupModel::getSpecialIdByPageId($pageConfig->pageId);
            $share = [];
            $domainKey = (new PageComponent())->getDomainKey();
            $siteConf = app()->params['sites'][ $pageData['site_code'] ][ $domainKey ];
            $domain = $siteConf[ $pageData['pipeline'] ][ $lang ];
            if (isset($pageData['pageLanguages'][0])) {
                $pageLang = $pageData['pageLanguages'][0];
                $share = json_decode($pageLang['share'], true);
                ...
                $pageConfig->background_attachment = $pageLang['background_attachment'];
                $pageConfig->goods_component_style = $pageLang['goods_component_style'];
                ...
                $pageConfig->share_image = $share['img'] ?? '';
                $pageConfig->share_title = $share['title'] ?? $pageLang['seo_title'];
                $pageConfig->share_desc = $share['desc'] ?? $pageLang['description'];
                $pageConfig->share_link = $domain . $pageLang['page_url'];
                $pageConfig->group_id = $pageLang['group_id'];
                $pageConfig->url_name = $pageLang['url_name'];
                $pageConfig->pipeline = $pageData['pipeline'];
            }
            ...
            $this->getShareDataString($share, $pageId, $data);
        }
    ...
    public function getHeadAndFooterByTplId($tplId, $lang)
    {
        ...
        if ($tpl = PageTplModel::findOne($tplId)) {
            ...
            $explainTpl = new ExplainTplComponent();
            $data = $explainTpl->getTplContent($pageConfig, app()->controller->module->id);
        }
     ...
     * @return array
     */
    private function getPageLayoutByPageId($pageId, $lang)
    {
        $pageInfo = PageModel::getPageInfo($pageId);
        if (!empty($pageInfo['activity_id'])) {
            $layoutList = $this->activityPageLayoutByPageId($pageId, $lang);
        } elseif ('home' === app()->controller->module->module->id) {
    ...
    private function activityPageLayoutByPageId($pageId, $lang)
    {
        return PageLayoutModel::find()->alias('l')
            ->select('l.*, d.data, d.custom_css, d.background_color, d.background_img, a.site_code, p.activity_id,p.pipeline')
            ->leftJoin(
    ...
    {
        return PageLayoutModel::find()->alias('l')
            ->select('l.*, d.data, d.custom_css, d.background_color, d.background_img, p.site_code, p.activity_id,p.pipeline')
            ->leftJoin(
                PageLayoutDataModel::tableName() . ' as d',
                'l.id = d.component_id'
            )->leftJoin(
     ...
     * @throws \yii\db\Exception
     */
    private function getPageUIByLayoutIds($layoutIds, $lang)
    {
    ...
    private function getUiList($layoutIds)
    {
        ...
        if (\is_array($layoutIds)) {
            $str = 'IN (' . implode(',', $layoutIds) . ')';
        } else {
            $str = is_numeric($layoutIds) ? '=' . $layoutIds : '';
            
        }
        
        $sql = /** @lang sql */
            'SELECT u.*, ui.need_navigate,p.pipeline FROM ' . PageUiModel::tableName() . ' AS u ';
        $sql .= ' LEFT JOIN ' . PageLayoutModel::tableName() . ' as l on  u.layout_id = l.id';
        $sql .= ' LEFT JOIN ' . PageModel::tableName() . ' as p on p.id = l.page_id';
        $sql .= ' INNER JOIN ui_component AS ui ON u.component_key=ui.component_key ';
        $sql .= 'WHERE u.layout_id ' . $str;
     ...
     * 解析Layout组件
     *
     * @param \app\modules\common\gb\components\CommonLayout $layout
    ...
    private function parseLayout($layout, $layoutArr, $uiData)
    {
        ...
        $layout->data = $uiData;
        $styleData = !empty($layoutArr['data']) ? json_decode($layoutArr['data'], true) : [];
        $layout->layoutData = !empty($styleData['layout_data']) ? $styleData['layout_data'] : [];
        $layout->customData = !empty($styleData['custom_data']) ? $styleData['custom_data'] : [];
     ...
     * 解析UI组件
     *
     * @param \app\modules\common\gb\components\CommonUi $ui
    ...
    private function getPageHtml($orderedLayouts, $uiListByLayoutPosition, $isPageTplExplain = false)
    {
        ...
        if (!empty($orderedLayouts)) {
            ...
            $layout->pipeline = $orderedLayouts[0]['pipeline'] ?? 'GB';
     ...
     * @param array                                      $uiListByLayoutPosition 按LayoutId和position分好组的UI组件列表
     * @param int                                        $layoutId               layout组件ID
     * @param \app\modules\common\gb\components\CommonUi $ui                     ui组件
     *
     * @return array
     * @throws \ego\base\JsonResponseException
     * @throws \yii\base\ViewNotFoundException
    ...
    public function getHeadExtraCss($pageId, $lang, $cssVersion, $siteCode, $activityPageType, $isPublish = false)
    {
        ...
        $css = /** @lang html */
            ...
            . Translate::getGbUiComponentJsTransMessage($lang)
            . '</script>';
        ...
        if (isGearbestSite($siteCode)) {
            $cssGridHtml = sprintf($cssGridFormat, $staticDomain, $siteCode, $cssFilenameSuffix, $cssVersion);
        } else {
    ...
    public function getHeadExtraJs($version, $lang, $siteCode, $activityPageType, $isPublish = false)
    {
        ...
        //是否为电子站点
        if (isGearbestSite($siteCode)) {
            $jsHtml .= sprintf($jsCommonFormat, $staticDomain, $siteCode, $jsFilenameSuffix, $version);
        } else {
     ...
     * @throws \yii\base\ViewNotFoundException
     */
    private function getOfflineHtml($siteCode, $pageId, $lang, $isAppSite = fasle, $pipeline)
    {
        $pageConfig = new CommonPageConfig();
        $pageConfig->lang = $lang;
        ...
        if ($isAppSite) {
            ...
            if ($tplData['isToHome']) {
                //跳转至首页
                $homeUrl = '/';
                if (isset(app()->params['sites'][ $siteCode ]['secondary_domain'][ $pipeline ][ $lang ])) {
                    $homeUrl = str_replace('promotion', '', app()->params['sites'][ $siteCode ]['secondary_domain'][ $pipeline ][ $lang ]);
                }
     ...
     * @return array
     */
    private function getUiDataByComponentId($list, $lang = '')
    {
        if (empty($list)) {
            return [];
        }
        ...
        $rs = PageUiComponentDataModel::find()
            ->select('`component_id`, `lang`, `key`, `value`, `tpl_id`, `is_public`')
            ->where($condition)
            ->asArray()
            ->all();
        if (empty($rs)) {
        ...
        foreach ($uilist as $key => $value) {
            foreach ($value as $k => $row) {
                foreach ($row as $y => $item) {
                    $goodsSKU = PageUiComponentDataModel::find()
                        ->select('`value`')
                        ->where([
                    ...
                    $data['goodsSKU'] = trim($goodsSKU['value'], '"');
     ...
     * @return void
     */
    private function getShareDataString($share, $page_id, &$html)
    {
        $key = 's3!#%$%%@##@@';
        $string = '';
        if (!empty($share) && is_array($share)) {
            foreach ($share['checked'] as $type) {
                $data = [
                    'id'                       => 100000 + (int) $page_id,
                    'share_type'               => $type,
                    'integral_effective_start' => (int) empty($share['integral_times'][0]) ? 0 : $share['integral_times'][0] / 1000,
                    'integral_effective_end'   => (int) empty($share['integral_times'][1]) ? 0 : $share['integral_times'][1] / 1000,
                    'integral_desc'            => $share['title'],
                    'share_integral'           => (int) $share['integral'],
                ];
                ksort($data);
                $str = '';
                foreach ($data as $val) {
                ...
                $string .= "<input type='hidden' id = 'geshop_share_" . $type . "' value='key=" . md5($key . $str) . "&content=" . json_encode($data) . "'>";
            }
            
            $string .= '<input type="hidden" id="shareChannel" value="' . join(',', $share['checked']) . '">';
            $pattern = '/<!--\s*geshop\s*main\s*end\s*-->/';
            preg_match($pattern, $html, $matches);
            if (!empty($matches[0])) {
                $html = str_replace($matches[0], $matches[0] . $string, $html);
            }
        }
    }
}
