...
use app\modules\activity\gb\components\PageDesignComponent;
...
trait CommonVerifyStatusTrait
{
    ...
    public function checkStatus($currentStatus, $changeToStatus)
    {
        ...
        return app()->helper->arrayResult(0, 'success', ['verify_status' => $this->verifyConfig[$key]]);
    }
    ...
    public function beforeVerifyPage($pageId, $status)
    {
        ...
        if(empty($pageModel->pipeline)){
            return app()->helper->arrayResult(1, '该页面没有设置渠道');
        }
