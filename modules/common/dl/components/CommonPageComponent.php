...
class CommonPageComponent extends Component
{
	...
	public function buildListData ($pageList, $pageLangList = null, $activity = null)
	{
		...
		foreach ($pageList as &$page) {
			...
			$page['urls'] = [];
			$page['design_url'] = Url::to(["/{$module}/dl/design/index", 'pid' => $page['pid']], true);
			if (!empty($activity)) {
			...
			$page['preview_url'] = Url::to([
				"/{$module}/dl/design/preview", 'pid' => $page['pid'], 'lang' => current($page['pageLanguages'])['lang']
			], true);
		}
	...
	public function refreshSitePage ($siteCode, $logId, $pageIdStrings)
	{
		...
		$lockKey = app()->redisKey->getDresslilySiteKey(
			app()->redisKey->getRefreshSiteTaskLockKey($siteCode, $module)
		);
	...
	public function refreshSiteLogDetail (int $id, int $status = 0)
	{
		...
		if (!empty($detail) && !empty($detail[0])) {
			$pageIds = array_unique(array_column($detail, 'page_id'));
