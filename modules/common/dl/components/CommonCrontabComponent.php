...
use app\modules\common\dl\models\{ActivityModel, PageModel, PageLanguageModel, PagePublishLogModel, SiteUpdateLogModel};
...
//            )->execute();
	...
	public function pushPage ()
	{
		...
		$key = app()->redisKey->getDresslilySiteKey(app()->redisKey->getPushTaskRedisKey());
				...
				if ($data) {
					...
					if ($putRes) {
						...
						if (empty($data['activity_id'])) {
							$homePageType = isset($data['home_page_type']) ? (int)$data['home_page_type'] : SiteConstants::HOME_PAGE_TYPE_INDEX;
							if ($homePageType === SiteConstants::HOME_PAGE_TYPE_INDEX_B) { // AB测试首页的B页处理
							...
							} else if ($homePageType === SiteConstants::HOME_PAGE_TYPE_INDEX) {
	...
	public function uiAutoRefresh ()
	{
		...
		$websiteCode = SiteConstants::SITE_GROUP_CODE_DL;
	...
	{
		$sitePlatform = ['dl-pc', 'dl-wap', 'dl-app'];
		$exceptionSite = [];
