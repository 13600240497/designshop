...
use app\modules\common\dl\models\{
    PageModel, PageUiDataModel, PageUiModel, PageLayoutModel, PageUiComponentDataModel, PageLanguageModel
};
...
class CommonPageUiDesignComponent extends CommonPageDesignComponent
{
    ...
    public function copyUiComponent($data)
    {
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            if(app()->controller->getRoute() == "activity/dl/ui-design/copy-ui"){
                //同步创建IPS活动参数构建
    ...
    public function addUiComponent($data)
    {
        ...
        $ui->pageId = $this->pageInfo->id;
        $ui->isVueSsr = 0;
        $tplId = UiModel::findOne(['component_key' => $data['component_key']])->getAttribute('tpl_id');
        if (!empty($tplId)) {
            $isVueSsr = UiTplModel::findOne(['id' => $tplId])->getAttribute('is_vue_ssr');
            $ui->isVueSsr = intval($isVueSsr);
        }
    ...
    public function saveForm($data)
    {
        ...
        // 清除特定组件的站点缓存数据
        $uiTplModel = UiTplModel::findOne(['id' => $data['tpl_id']]);
        if ($uiTplModel->component_key === 'U000203' && $uiTplModel->name_en === 'template3') {
            $this->cleanSiteGoodsCache($data['site_code']);
        }
        ...
        $ui->activityId = $this->pageInfo->activity_id;
        $ui->pageId = $this->pageInfo->id;
        $ui->isVueSsr = !empty($data['is_vue_ssr']) ? intval($data['is_vue_ssr']) : 0;
        if (!$componentHtml = $ui->renderComponent($ui)) {//组件解析
            throw new JsonResponseException($this->codeFail, $ui->errors);
        }
     ...
     */
    protected function cleanSiteGoodsCache($siteCode)
    {
        $options = [
            RequestOptions::FORM_PARAMS => ['content' => '{"key":"goodsTabList"}']
        ];
        $apiUrl = app()->params['sites'][ $siteCode ]['goods_delCache']['url'] ?? null;
        ...
        try {
            ...
            \Yii::info(sprintf('请求接口 %s 成功，返回: %s', $apiUrl, $body), __METHOD__);
        } catch (\Exception $e) {
            \Yii::error(sprintf('请求接口 %s 异常，错误: %s', $apiUrl, $e->getMessage()), __METHOD__);
            throw new JsonResponseException($this->codeFail, '站点清除商品信息接口请求异常！');
        }
    }
