...
class CommonConvertAppComponent extends Component
{
     ...
     * App端默认layout组件key
     */
    const DEFAULT_LAYOUT_KEY = 'L000054';
    ...
    private function checkConvertParamsAndAuth(array $params)
    {
        ...
        if ((int)$sourcePageModel->type !== ActivityModel::TYPE_WEB) {
            throw new JsonResponseException($this->codeFail, '源页面非Web端页面');
        }
     ...
     * @throws \Exception
     */
    public function webConvertApp(array $params)
    {
        ...
        try {
            ...
            $sourceLangCode = explode(',', !empty($params['lang']) ? $params['lang'] : $targetPageModel->lang);
            //取源页面数据
            $componentData = $this->getSourcePageData($sourcePageModel->id, $sourceLangCode);
            ...
            foreach ($targetPageModelList as $targetPageModel) {
                $page_layout = PageLayoutModel::find()->where(['page_id' => $targetPageModel->id,"lang"=>$sourceLangCode])->asArray()->one();
            ...
            return app()->helper->arrayResult(
                ...
                $this->msgSuccess,
                [
                    'redirectUrl' => Url::to(['/activity/dl/design/index', 'pid' => $targetPageModel->pid], true),
                    'siteCode' => $targetPageModel->site_code
                ]
            );
        } catch (Exception $e) {
    ...
    private function copyPageUiComponent(array $uiParams, int $layoutId, int &$uiFirstId)
    {
        ...
        if (!empty($uiArr)) {
            $nextId = 0;
            foreach ($uiArr as $ui) {
                /** @var \app\modules\common\dl\models\PageUiModel $uiModel */
