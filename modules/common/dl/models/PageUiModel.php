...
class PageUiModel extends AbstractBaseModel
{
    ...
    {
        return static::getDLTableName('page_ui_component');
    }
    ...
    {
        $moduleId = app()->controller->module->module->id;
        if (in_array(strtolower($moduleId),['activity','advertisement'])) {
            return self::activitySiteCode($uiId);
        }
        if ('home' === strtolower($moduleId)) {
