...
class ActivityModel extends AbstractBaseModel
{
    ...
    //活动类型|7web端
    const TYPE_WEB = SitePlatform::PLATFORM_TYPE_WEB;
    ...
    public function rules()
    {
        return [
            [
                'type',
                'in',
                'range'   => [SitePlatform::PLATFORM_TYPE_WEB, SitePlatform::PLATFORM_TYPE_APP],
                'message' => '{attribute}只能是Web、APP端'
            ],
            [['group_id', 'type', 'site_code', 'lang', 'name'], 'required'],
    ...
    public static function getActivityInfo($activityId)
    {
        ...
        if ($model) {
            $data = ArrayHelper::toArray($model);
            $data['langList'] = static::getLangListByLangString($data['lang']);
        }
     ...
     * @return bool
     */
    public static function checkUrlNameExist($activityId, $pageId, $urlName, $lang)
    {
