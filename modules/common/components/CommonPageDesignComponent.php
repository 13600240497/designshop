...
use app\components\site\fz\PagePreview;
...
class CommonPageDesignComponent extends Component
{
    ...
    public $uiKeyIncludeSku = [
        'U000001', 'U000031', 'U000041', 'U000191', 'U000192', 'U000078', 'U000141', 'U000153', 'U000209',
        'U000236', 'U000237', 'U000109', 'U000110'
    ];
    ...
    public function init()
    {
        ...
        $this->authCheckIgnoreRoute = [
            ...
            'gbad/design/index',
            'gbad/design/preview',
            'activity/design/snapshot',
            'home/design/snapshot',
            'advertisement/design/snapshot',
            'gbad/design/snapshot'
        ];
        $this->changeRefreshIgnoreRoute = [
            'activity/design/index',
            'activity/design/preview',
            'activity/design/release',
            'activity/design/get-setting',
            'activity/layout-design/get-layout-form',
            'activity/ui-design/get-form',
        ];

        if (!empty($_REQUEST[ static::FIELD_PAGE_ID ]) && in_array(strtolower(app()->controller->module->id),['activity','advertisement','gbad'])) {
            $this->pageInfo = PageModel::getPageActivityInfo((int) $_REQUEST[ static::FIELD_PAGE_ID ]);
        }
        if (!empty($_REQUEST[ static::FIELD_PAGE_ID ]) && 'home' === app()->controller->module->id) {
    ...
    public function checkAuth($pageId, $route, $lang='')
    {
        ...
        $redisKey = app()->redisKey->getDesignPageLockKey($pageId, $lang);
     ...
     * @throws \Throwable
     */
    public function activityRelease($pageId, $lang, $type='', $langList='', $isAd = false)
    {
        ...
        if (PageModel::PAGE_STATUS_HAS_ONLINE === $pageModel->status) {
	        if (!empty($pageModel->activity_id) && false === $isAd) {
		        try {
            ...
            list($success, $errorMsg) = $this->batchCreateOnlinePageHtml([$pageId], $pageModel->activity_id
                ,false, false, false, $type, false, $langList);
            if (!$success) {
    ...
    public function homeOnline($pageId, $lang, $langList)
    {
        ...
        try {
            ...
            if ($pageModel::PAGE_STATUS_HAS_PUSH === $pageModel->status) {
                $langList = !empty($langList) ? explode(',',$langList) : [];
                list($success, $errorMsg) = $this->batchCreateOnlinePageHtml([$pageId], 0, false, false, false, false, false, $langList);
                if (!$success) {
    ...
    public function homeOnlineB($pageId, $lang, $langList)
    {
        ...
        try {
            ...
            if ($pageModel::PAGE_STATUS_HAS_PUSH === $pageModel->status) {
                $langList = !empty($langList) ?  explode(',',$langList) : [];
                list($success, $errorMsg) = $this->batchCreateOnlinePageHtml([$pageId], 0, false, false, false, '', true, $langList);
                if (!$success) {
    ...
    public function homeRelease($pageId)
    {
        ...
        if ($pageModel->status == PageModel::PAGE_STATUS_HAS_ONLINE) {
            ...
            // 正在使用时，发布操作不修改状态
            $finalStatus = [
                PageModel::PAGE_STATUS_HAS_PUSH,
                PageModel::PAGE_STATUS_HAS_ONLINE_UPDATE,
                PageModel::PAGE_STATUS_HAS_ONLINE_B
            ];
            if (!in_array($pageModel->status, $finalStatus)) {
    ...
    {
        $key = app()->redisKey->getPageTplRedisKey($id);
