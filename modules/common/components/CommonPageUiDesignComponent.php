...
use app\modules\common\models\{
    PageUiDataModel, PageUiModel, PageLayoutModel, PageUiComponentDataModel,PageLanguageModel
};
...
class CommonPageUiDesignComponent extends CommonPageDesignComponent
{
    ...
    public function copyUiComponent($data)
    {
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            if(app()->controller->getRoute() == "activity/ui-design/copy-ui"){
                //同步创建IPS活动参数构建
    ...
    public function getForm($pageUiId, $lang, $tplId = 0)
    {
        ...
        $navNewData = [];
        $i = 0 ;
        foreach ($ui->data['navData'] as $key => $item) {
            if (!isset($ui->data['navNewData'])) {
                $navNewData[$i] = ['id' => $key, 'name' => $item, 'img' => ''];
            } else {
                $ui->data['navNewData'][$i]['name'] = $item;//动态查询更新标题栏文案
                if ($ui->data['navNewData'][$i]['id'] != $key) {//判断是从别的渠道复制过来的数据
                    $ui->data['navNewData'][$i]['id'] = $key;
                }
            }
            $i++;
        }
        if (empty($ui->data['navData'])) {
            $ui->data['navNewData'] = [];
        }
        $ui->data['navNewData'] = empty($navNewData) ? $ui->data['navNewData'] : $navNewData;
