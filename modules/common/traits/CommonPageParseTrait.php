...
use app\modules\common\models\{
    ActivityModel, PageModel, PageLanguageModel, PagePublishCacheModel, PageLayoutModel, PageLayoutDataModel, PageUiModel, PageTplModel, PageUiComponentDataModel
};

use app\modules\common\components\{
    CommonPageConfig, CommonUi, CommonLayout
};

use app\modules\component\components\{
...
trait CommonPageParseTrait
{
    ...
    {
        if (in_array(app()->controller->module->id,['activity','advertisement','gbad']) ) {
            $layoutList = $this->activityPageLayoutByPageId($pageId, $lang);
        } elseif ('home' === app()->controller->module->id) {
    ...
    private function activityPageLayoutByPageId($pageId, $lang)
    {
        return PageLayoutModel::find()->alias('l')
            ->select('l.*, d.data, d.custom_css, d.background_color, d.background_img, d.style_data, a.site_code, p.activity_id')
            ->leftJoin(
    ...
    {
        return PageLayoutModel::find()->alias('l')
            ->select('l.*, d.data, d.custom_css, d.background_color, d.background_img, d.style_data, p.site_code, p.activity_id')
            ->leftJoin(
                PageLayoutDataModel::tableName() . ' as d',
                'l.id = d.component_id'
            )->leftJoin(
    ...
    private function getUiList($layoutIds)
    {
        ...
        $sql = /** @lang sql */
            'SELECT u.*, ui.need_navigate FROM page_ui_component AS u ';
    ...
    public function getHeadExtraCss($pageId, $lang, $cssVersion, $siteCode, $activityPageType, $isPublish = false)
    {
        ...
        //GB组件多语言 - 兼容GB站点广告落地页面，后续删除
        $transMessage = isGearbestSite($siteCode) ? Translate::getGbUiComponentJsTransMessage($lang)
        ...
        $isDirectUseApiFallbackData = (int)SysConfigUtils::get('sys.ui.rg.direct_use_api_fallback_data', 0);
    ...
    public function getHeadExtraJs($version, $lang, $siteCode, $activityPageType, $isPublish = false)
    {
        ...
        $platformCode = SitePlatform::getPlatformCodeBySiteCode($siteCode);
        $platformJsSuffix = (SitePlatform::PLATFORM_CODE_PC != $platformCode) ? 'm' : 'pc';
        $js_laytpl = sprintf('<script src="%s/resources/javascripts/library/gs_laytpl.js?version=%s"></script>', $staticDomain, $version);
     ...
     * @return array
     */
    public function getInterfaceConfig($siteCode, $isJsonp=true)
    {
     ...
     * 获取可选的新的导航菜单
     *
     * @param array $navNewData
     * @param array $navData
     * @return array
     */
    public function getAvailableNewNavigation($navNewData, $navData)
    {
        if (empty($navData)) {
            return [];
        }
        ...
        foreach ($navData as $key => $item) {
            if (empty($navNewData)) {
                $navNewData[$i] = ['id' => $key, 'name' => $item, 'img' => ''];
            } else {
                $navNewData[$i]['name'] = $item;//动态查询更新标题栏文案
                if ($navNewData[$i]['id'] != $key) {//判断是从别的渠道复制过来的数据
                    $navNewData[$i]['id'] = $key;
                }
            }
            $i++;
