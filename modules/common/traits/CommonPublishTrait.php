...
use app\modules\common\components\CommonCrontabComponent;
...
trait CommonPublishTrait
{
		...
		if (!empty($pageIds)) {
			...
			foreach ($pageIds as $pageId) {
				if ($createType === $this->createTypeOnline
					&& !$this->createOnlinePageHtml($langList, $pageId, $useCache, $updateGoods, $updateUiGoods, $type, $isHomeB)
				) {
					$errorCount++;
				}
	...
	private function createOnlinePageHtml ($langList, $pageId, $useCache, $updateGoods = false, $updateUiGoods = false, $type = '', $isHomeB = false)
	{
		if (!empty($langList)) {
			...
			!empty($createResult) && $this->saveCreateResult($createResult, $type);
		}
		...
		foreach ($params as list($pageId, $lang, $logData, $redisData, $createRes)) {
			...
			if ($pageLangModel = PageLanguageModel::findOne(['page_id' => $pageId, 'lang' => $lang])) {
				...
				if (!empty($redisData)) {
					...
					if (empty($this->activityInfo->id)
						|| $this->activityInfo->mold == SiteConstants::ACTIVITY_PAGE_TYPE_ADVERTISEMENT
						|| (defined('PUSHSYNC') && false === PUSHSYNC)) {//首页发布走redis队列
	...
	private function createStatics ($params, &$redisData, &$logData)
	{
		...
		if (!isGearbestSite($siteCode)) {
			...
			if (file_exists($platformCss)) {
				$result['css'] = file_get_contents($platformCss) . ($result['css'] ?? '');
			}
		}
	...
	private function createFile ($params, &$redisData, &$logData)
	{
		...
		$localPath = $this->getLocalPath([$pageId, $lang, $fileType, $siteCode, $this->activityInfo->mold, $isHomeB]);

		if (false === defined('PUSHSYNC') || (true === defined('PUSHSYNC') && false === PUSHSYNC)) {
			//内容写入Redis中
			$redisKey = app()->redisKey->getPublishContentKey([$pageId, $lang, $fileType, $version]);
	...
	private function pushTaskToRedis ($data)
	{
		if (!empty($data) && \is_array($data)) {
			$redisLength = app()->redis->rpush(app()->redisKey->getPushTaskRedisKey(), ...$data);
	...
	private function buildHtmlWithJsAndCss ($params)
	{
		...
		$htmlPageUrl = $this->getPageLangUrl([$pageId, $lang, $this->htmlFileType, $siteCode, $mold, $isHomeB]);
	...
	private function getS3UrlByS3Key ($params)
	{
		...
		if ($fileType === $this->htmlFileType) {
			...
			if (SiteConstants::ACTIVITY_TYPE_GB_ADVERTISEMENT == $mold) {
	...
	{
		list($pageId, $lang, $fileType, $siteCode, $mold, $isHomeB) = $params;
		$localPath = $this->getLocalPath([$pageId, $lang, $fileType, $siteCode, $mold, $isHomeB]);
		$s3Key = $this->getS3KeyByLocalPath($localPath, $fileType);
	...
	private function getLocalPath ($params)
	{
		...
		if ('home' === app()->controller->module->id) {
			$urlName = $isHomeB ? 'index_new' : 'index';
		} else {
	...
	private function getStaticsFilePathPreOnS3 ($siteCode, $lang)
	{
		...
		if (isGearbestSite($siteCode)) {
			$s3StaticPathKey = 'ad_s3StaticPath';
		}
