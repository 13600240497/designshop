...
trait CommonConvertTrait
{
    ...
    public function getConvertRelationList(int $pageId, string $lang, string $siteCode)
    {
        ...
        if (!empty($site[1]) && $currentActivity = PageModel::findOne($pageId)) {
            $list[$site[1]]['url'] = $this->getPageDesignUrl($currentActivity->pid, $lang);
            if ($site[1] === 'pc') {
                $this->getWapAndAppByPc($pageId, $lang, $list);
            } elseif ($site[1] === 'wap') {
                $this->getPcAndAppByWap($pageId, $lang, $list);
            } elseif ($site[1] === 'app') {
                $this->getPcAndWapByApp($pageId, $lang, $list);
            }
        } else {
            $list = [];//都没有则置空
        }
     ...
     * @throws \yii\base\InvalidArgumentException
     */
    private function getWapAndAppByPc(int $pageId, string $lang, array &$list)
    {
        //查询wap
        if ($wapPage = PageConvertRelationModel::getLastTargetPid($pageId)) {
            $list['wap']['url'] = $this->getPageDesignUrl($wapPage->pid, $lang);

            //查询app
            if ($appPage = PageConvertRelationModel::getLastTargetPid($wapPage->page_id)) {
                $list['app']['url'] = $this->getPageDesignUrl($appPage->pid, $lang);
            } else {
     ...
     * @throws \yii\base\InvalidArgumentException
     */
    private function getPcAndAppByWap(int $pageId, string $lang, array &$list)
    {
        //查询pc
        if ($pcPage = PageConvertRelationModel::getLastSourcePid($pageId)) {
            $list['pc']['url'] = $this->getPageDesignUrl($pcPage->pid, $lang);
        } else {
     ...
     * @throws \yii\base\InvalidArgumentException
     */
    private function getPcAndWapByApp(int $pageId, string $lang, array &$list)
    {
        //查询wap
    ...
    {
        return Url::to([
            '/'.app()->controller->module->id.'/design/preview',
            'pid'  => $pid,
            'lang' => $lang
        ], true);
    }
     ...
     * @throws \yii\base\InvalidArgumentException
     */
    private function getPageDesignUrl(string $pid, string $lang)
    {
        return Url::to([
            '/'.app()->controller->module->id.'/design/index',
            'pid'  => $pid,
            'lang' => $lang
        ], true);
    }
