 ...
 */
class PageLanguageModel extends ActiveRecord
{
    /** @var int 页面样式 - 系统设置 */
    ...
    {
        if (empty($params['source_id']) || empty($params['target_id']) || empty($params['lang'])) {
            throw new Exception("参数错误，同步商品类组件样式失败");
        }
        
        $goodsComponentStyle = static::find()
            ->where(['page_id' => $params['source_id'], 'lang' => $params['lang']])
            ->one()
        ...
        if (!empty($params['lang'])) {
            static::updateAll(
                ...
                ['page_id' => $params['target_id'], 'lang' => $params['lang']]
            );
        }
    }
