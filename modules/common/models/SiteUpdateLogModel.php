 ...
 */
class SiteUpdateLogModel extends ActiveRecord
{
    /**
     * 状态|0-未开始
     */
	 ...
	 */
	public static function getSiteCurrentDateLog (string $siteCode)
	{
		return self::find()->select('id, page_ids, status, create_user, create_time')
			->where(['site_code' => $siteCode])
			//->andWhere('create_time >= ' . strtotime(date('Y-m-d')))
			->orderBy('id DESC')
			->one();
	}
}
