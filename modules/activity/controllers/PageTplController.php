...
/**
 * 页面模板管理类
 * @property \app\modules\activity\components\PageTplComponent $PageTplComponent
...
class PageTplController extends Controller
{
    ...
    {
        $site_code = app()->request->post('site_code','');
        $action = app()->request->post('action','activity');
        $url = '"';
        $url .= app()->params['url']['admin'];
        $site = SitePlatform::getSiteBySiteCode($site_code);
        switch ($action){
            case 'home':
                $url .= '/home';
                break;
        ...
        $url .= '/design/snapshot?page_id='.app()->request->post('id','') . '&is_client=1&site_group_code=' . $site;
        $url .= '"';
