...
use app\modules\activity\traits\PublishTrait;
...
class PageComponent extends CommonPageComponent
{
    ...
    public function lists($activityId)
    {
        ...
        if (($activity = ActivityModel::findOne($activityId)) && !empty($activity->lang)) {
            $configSiteLanguages = app()->params['sites'][ $activity->site_code ][ $this->getDomainKey() ] ?? [];
            $siteLanguageKeys = array_keys($configSiteLanguages);
            foreach (explode(',', $activity->lang) as $item) {
                if(!in_array($item,$siteLanguageKeys)){
                    continue;
                }
                $langList[] = [
                    'key'  => $item,
        ...
        $themeList = $ObsComponent->getThemeByActivity($activityId); //obs选择活动
    ...
    public function groupAdd(array $params)
    {
        ...
        try {
            ...
            foreach ($validPlatformParams as $platformCode => $platformParams) {
                ...
                $this->beforeSaveObs($platformParams,[$pageModel->id,
    ...
    public function groupEdit(array $params)
    {
        ...
        try {
            ...
            foreach ($validPlatformParams as $platformCode => $platformParams) {
                ...
                $this->beforeSaveObs($platformParams,
                    [
                        $platformParams['page_id'],
                        SitePlatform::getPlatformTypeByPlatformCode($platformCode),
    ...
    private function beforeSaveProcessor($validPlatformParams, &$platformParams)
    {
        ...
        if (SitePlatform::isPcPlatform($platformParams['site_code'])
            && isset($validPlatformParams[SitePlatform::PLATFORM_CODE_WAP])) {
            $wapSiteCode = $validPlatformParams[SitePlatform::PLATFORM_CODE_WAP]['site_code'];
            $allSupportLanguages = SitePlatform::getSiteSpecialPageSupportLanguages($wapSiteCode);
            $allSupportLanguages = array_column($allSupportLanguages, NULL, 'key');
            foreach ($platformParams['list'] as $lang => $langData) {
                if(empty($platformParams['list'][$lang]['redirect_url'])){
                    $mobileActivityUrl = '';
                    if (isset($allSupportLanguages[$lang])) {
                        $mobileActivityUrl = trim($allSupportLanguages[$lang]['url'], '/') .'/';
                        $mobileActivityUrl .= $validPlatformParams[SitePlatform::PLATFORM_CODE_WAP]['list'][$lang]['url_name'];
    ...
    {
        $ObsComponent = new ObsComponent();
        list($page_id,$platform,$activity_id) = $params;
