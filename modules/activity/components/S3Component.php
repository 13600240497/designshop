...
{
    private $syncDirs = ['resources', 'frontend'];
    /**
     * @var string 待同步的目录名
     */
    private $currentSyncDir = 'resources';
    ...
    {
        $key = app()->redisKey->getSyncResourcesRedisKey();
        ...
        foreach ($this->syncDirs as $syncDir) {
            $this->currentSyncDir = $syncDir;
            $resourceDir = app()->basePath . DIRECTORY_SEPARATOR . 'htdocs' . DIRECTORY_SEPARATOR . $this->currentSyncDir;
    ...
    private function getS3KeyByPath($path)
    {
        ...
        return app()->params['s3PublishConf']['staticKeyPre'] . '/' .
            implode(DIRECTORY_SEPARATOR, \array_slice($explode, array_search($this->currentSyncDir, $explode, true)));
    }
