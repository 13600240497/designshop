...
use app\modules\common\dl\components\CommonPageUiTplComponent;
...
class PageUiTplComponent extends CommonPageUiTplComponent
{
    ...
    {
        try {
            $url = $this->uploadTplPicToS3();
            return app()->helper->arrayResult($this->codeSuccess, $this->msgSuccess, ['url' => $url]);
        } catch (BizException $e) {
            return app()->helper->arrayResult($this->codeFail, $e->getMessage());
        }
    }
