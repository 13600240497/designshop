...
<input type="text" name="templateName" class="layui-input" placeholder="组件筛选">
                                ...
                                <?php foreach ($activityInfo['allLangList'] as $outerItem): ?>
                                    ...
                                    <select name="" lay-filter="selectLang">
                                        <?php foreach ($outerItem['langList'] as $item) { ?>
                                            <option value="<?= \yii\helpers\Url::current(['lang' => $item['key']]); ?>" <?= $lang !== $item['key'] ? '' : 'selected'; ?>><?= $item['name']; ?><?php if ($item['is_default'] == 1) { ?><span style="color: #999;">(默认)</span><?php } ?></option>
                                        <?php } ?>
                                    </select>
                                    <?php } else {
    ...
    <div id="design_view_loading" class="design-view-loading-v2">
