...
class ExplainTplComponent extends Component
{
    ...
    public function getTplContent($pageConfig, $place = '')
    {
        ...
        //GB临时方案-尾部banner隐藏 - 兼容GB站点广告落地页面，后续删除 - 2018-11-21
        ...
        if (!empty($matches[0])) {
            ...
            //GB大数据埋点 - 兼容GB站点广告落地页面，后续删除
            if (isGearbestSite($pageConfig->siteCode)) {
                $commonJavascript = '/<!--\s*Common\s*JavaScript\s*-->/';
                preg_match($commonJavascript, $html, $matches);
                if(!empty($matches[0])) {
                    $pipeline = \app\modules\gbad\utils\GearbestCompatibleUtils::getPipelineByLang($pageConfig->lang);
                    $bigDataJs = (new \app\modules\component\gb\components\BigDataComponent())->bulit(
                        $pageConfig->pageId, $pageConfig->lang, $pipeline, $pageConfig->siteCode
                    );
        ...
        if (!empty($api) && filter_var($api, FILTER_VALIDATE_URL, FILTER_FLAG_PATH_REQUIRED)) {
            try {
                ...
                if (app()->env->isPreRelease()) {
                    //GB获取配置域名 - 兼容GB站点广告落地页面，后续删除
                    $_domain = isGearbestSite($siteCode)
                        ? \app\modules\gbad\utils\GearbestCompatibleUtils::getConfigDomain($siteCode, $lang)
                        : app()->params['sites'][ $siteCode ]['domain'];
