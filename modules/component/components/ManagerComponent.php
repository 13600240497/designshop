...
class ManagerComponent extends Component
{
    ...
    *组件logo图片目录
    ...
    public function add($type, $data)
    {
	    ...
	    $this->componentModel->icon = $data['icon'] ?? '';
    ...
    public function edit($id, $type, $data)
    {
        ...
        if (!empty($data['range']) && ((int)$data['range'] === UiModel::RANGE_RESPONSIVE)) {
            $_siteGroups = explode(',', $data['siteGroups']);
            foreach ($_siteGroups as $_siteGroupCode) {
                $supportResponsive = app()->params['site'][ $_siteGroupCode ]['support_responsive'] ?? false;
                if (!$supportResponsive) {
                    $this->errors = sprintf('站点 %s 不支持响应式组件', $_siteGroupCode);
                    return false;
                }
            }
        }
    ...
    public function getList(array $params)
    {
        ...
        $tplList = $this->getComponentTplList($componentKeys, $status);
    ...
    public function isOnline($key)
    {
        ...
        $res = $this->componentModel->query
            ...
            ->one();
        return (int)$res->status === ComponentModel::STATUS_USED;
    }
     ...
     * @return bool
     */
    public function hasOnlineTpl($componentKey)
    {
        $condition = [
            'component_key' => $componentKey,
            'status' => UiTplModel::STATUS_USED,
            'is_delete' => UiTplModel::NOT_DELETE,
        ];
        $count = UiTplModel::find()->where($condition)->count();
        return $count > 0;
    }
    ...
    public function changeStatus($key, $status)
    {
        ...
        try {
            // 下线组件时同时下线组件下所有模板
            if (ComponentModel::STATUS_OFFLINE === (int)$status) {
                $componentTpl = new ComponentTplComponent();
                $componentTpl->offlineComponentAllTpl($key);
            }

            $res->status = $status;
            $res->update_user = app()->user->username;
            if (!$res->save()) {
                throw new Exception('更新组件失败！');
            }
            ...
            $this->errors = sprintf('更新失败,错误：%s', $e->getMessage());
            return false;
        }
    }
    ...
    private function checkValid($info, $status)
    {
        ...
        if (isset($info['range']) && 4 === $info['range']) {
            return true;
        }
