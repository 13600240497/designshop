...
class PageComponent extends CommonPageComponent
{
    ...
    private function checkAndBuildData($activityModel, $pageId, $params, $postLangDataList)
    {
        ...
        $langDataFieldName = ['title', 'goods_sn', 'description', 'statistics_code','page_url'];
            ...
            foreach ($supportPlatformList as $platformCode) {
                foreach ($langData as $key => $value) {
                    if (in_array($key, $langDataFieldName, true)) {
                        if($key == 'page_url' && strpos($value,'.com/') !== false){
                            $value = '/'.explode('.com/', $value)[1];
                        }
        ...
        foreach ($allPlatformParams as $platformCode => $platformParams) {
            ...
            foreach ($platformParams['list'] as $langKey => $pageLangInfo) {
                ...
                if (empty($pageLangInfo['goods_sn'])) {
                    $errorMsg = sprintf("应用端口 %s 产品编码为必填项", SitePlatform::getPlatformNameByCode($platformCode));
                    throw new JsonResponseException($this->codeFail, $errorMsg);
                }
    ...
    public function getGoodsUrls($goods_sn, $lang, $platform, $page_id=0)
    {
        ...
        if(empty($data['code'])){
            ...
            throw new JsonResponseException($this->codeFail, '获取失败,请检查产品编码是否正确');
        }else{
    ...
    public function getPageNewestUrls(int $pageId, int $needBtn = 1)
    {
        ...
        $languageList = PageLanguageModel::find()->where(['page_id'=>$pageId])
        ->select('status,lang,page_url')->asArray()->all();
        if (!$languageList) {
            throw new JsonResponseException($this->codeFail, '数据错误，未找到页面记录', $data);
        }
        $data['total'] = count($languageList);
        ...
        foreach ($languageList as $row) {
            if ($row['status'] == 1) {
                $domain = str_replace('/promotion', '', $siteConf[ $domainKey ][ $row['lang'] ]);
                $data['list'][] = [
                    'lang'      => $row['lang'],
                    'lang_name' => $langConf[ $row['lang'] ]['name'],
                    'page_url'  => $domain . $row['page_url']
                ];
            } else {

                $tips[] = $langConf[ $row['lang'] ]['name'];
            }
        }
        if (!empty($tips)) {
