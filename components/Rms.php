...
use yii\base\ErrorException as PHPErrorException;
...
class Rms extends Component
{
    ...
    /**
     * @var string 推送S3错误监控简码
     */
    const ERROR_S3 = 'SNB40888';
    /**
     * @var string 站点头尾部内容变更监控简码
     */
    const ERROR_HF = 'WFZ72512';
    
    /**
     * @var string 第三方平台接口调用监控简码
     */
    const ERROR_API = 'SJC33082';
    ...
    public static $code2name = [
        ...
        self::ERROR_S3    => '推送S3错误监控',
        self::ERROR_HF    => '站点头尾部内容变更监控',
        self::ERROR_API   => '第三方平台接口调用监控'
    ];
    
    /**
     * @var array 自定义监控错误码映射信息
     *
     * @var array
     */
    public static $customizeErrCode = [
        self::ERROR_S3  => 290301,
        self::ERROR_HF  => 270401,
        self::ERROR_API => 290302
    ];
     ...
     * @param int $isTest
     */
    public function reportS3PushError($error, $isTest = 0)
    {
        $this->reportCustomizeError($error, self::ERROR_S3);
    }
     ...
     * @param int $isTest
     */
    public function reportHeadFooterError($error, $isTest = 0)
    {
        $this->reportCustomizeError($error, self::ERROR_HF, self::LEVEL3);
    }
     ...
     * @param $error
     */
    public function reportPlatformApiError($error)
    {
        $this->reportCustomizeError($error, self::ERROR_API);
    }
     ...
     * @param int $level
     */
    public function reportCustomizeError($error, $code, $level = self::LEVEL2)
    {
        $data['server_name'] = $this->getServeIdentifies();
        $serverIpTemp = explode('_', $data['server_name']);
        $data['server_ip'] = end($serverIpTemp);
        // 手动获取北京时区时间戳
        $data['notice_time'] = app()->datetime->cn->format('Y-m-d H:i:s');
        /**
         * 错误明细格式
        ...
        if (app() instanceof \yii\console\Application) {
            ...
            $errorDetail = [
                $error,
                'REQUEST URL[' . app()->request->hostInfo . app()->request->getUrl() . ']',
        ...
        $data['content'] = [
            // 40000字以内
            'info' => implode('<br />', $errorDetail)
        ];
        
        $errorInfo = ['code' => self::$customizeErrCode[ $code ], self::ERROR_LEVEL => $level];
        $buildData = $this->buildErrorData($code, $errorInfo, $data);
        // 监控异常数据加入Redis
        $this->pushToRedis($buildData, $code, array_slice($errorDetail, 0, 3));
    }
    ...
    private function observeProgrammerException($exception)
    {
        ...
        } elseif ($exception instanceof PHPErrorException || $exception instanceof \Error
            || $exception instanceof Exception
        ) {
            // exception of PHP
            // PHP在7.0版本以上错误会抛出为Error类
            // 我们YII系统所有PHP错误都会抛出异常，显示错误页面，破坏程序流程，因而都定义为错误
            $errorInfo = ['code' => 100001, self::ERROR_LEVEL => self::LEVEL2];
            $pointCode = self::ERROR_PHP;
        } else {
            $errorInfo = ['code' => 100001, 'level' => self::LEVEL2];
            $pointCode = self::ERROR_PHP;
        }
    ...
    private function buildErrorData($pointCode, $errorInfo, $data)
    {
        ...
        \yii::info(['message' => 'push to Redis', $buildData], __METHOD__);
     ...
     * 设置获取服务信息
     *
     * @return BaseResponseCurl
     */
    protected function server()
    {
        if (empty($this->server)) {
            $this->server = new BaseResponseCurl();
        }
    ...
    protected function pushToRedis(array $data, $pointCode, array $salt)
    {
        if (isset($data['point_code'])) {
            ...
            $item = app()->redis->hget($this->redisKey, $key);
            $item || app()->redis->hset($this->redisKey, $key, json_encode($data));
        }
    }
    ...
    {
        $result = app()->redis->hgetall($this->redisKey);
        if (!count($result)) {
            return 'not data to Consume';
        }
        $response = false;
        foreach ($result as $item) {
            $item && ($response = $this->serverPost($item, false));
        }
        app()->redis->del($this->redisKey);
        
        return !empty($response) ? $response->getBody() . '' : 'response get is null, not data to Consume';
    }
     ...
     * @param bool  $toJson 是否JSON_ENCODE数据
     *
     * @return Response|null
     */
    private function serverPost($data, $toJson = true)
    {
        if (app() instanceof \yii\console\Application) {
            return (new Client())->request(
                ...
                [
                    'form_params' => [
                        'token'  => md5($this->token . ($toJson ? json_encode($data) : $data)),
                        'encode' => 1,
                        'data'   => base64_encode(($toJson ? json_encode($data) : $data)),
                    ],
                ]
            );
        } else {
            return $this->server()->slient()->request(
                'POST',
                $this->url,
                [
                    'form_params' => [
     ...
     */
    public function mysqlSelfCheck()
    {
        return $this->check(
            self::ERROR_MYSQL,
            [
                'code'            => self::$mysqlCode2CustomCodeInfo[1049]['code'],
                self::ERROR_LEVEL => self::$mysqlCode2CustomCodeInfo[1049][ self::ERROR_LEVEL ],
            ],
            '错误来自监控自检脚本: 测试MYSQL ERROR_MYSQL报错'
        );
    }
     ...
     * @return Response|null
     */
    public function phpSelfCheck()
    {
        return $this->check(
            self::ERROR_PHP,
            [
                'code'            => self::$phpCode2CustomCodeInfo[ E_ERROR ]['code'],
                self::ERROR_LEVEL => self::$phpCode2CustomCodeInfo[ E_ERROR ][ self::ERROR_LEVEL ],
            ],
            '错误来自监控自检脚本: 测试PHP E_ERROR_PHP报错'
        );
    }
     ...
     * @return Response|null
     */
    public function swooleSelfCheck()
    {
        return $this->check(
            self::ERROR_S3,
            [
                'code'            => self::$customizeErrCode[ self::ERROR_S3 ],
                self::ERROR_LEVEL => self::LEVEL2,
            ],
            '错误来自监控自检脚本: 测试SWOOLE推送任务进程报错'
        );
    }
     ...
     * 自检错误 -- 五分钟一次
     *
     * @param string $pointCode 监控点名称
     * @param array  $errorInfo 监控的错误代码code和级别level
     *                          ```
     *                          array( 'code' => '2323', self::ERROR_LEVEL => 2)
     *                          ```
     * @param string $info
     * @param bool   $isTest    是否是测试
     *
     * @return Response|null
     */
    public function check($pointCode, array $errorInfo, $info = '', $isTest = true)
    {
        $serverIdentifies = $this->getServeIdentifies();
        $serverIpTemp = explode('_', $serverIdentifies);
        $data = [
            'point_code'      => $pointCode,
            'error_code'      => $errorInfo['code'] ?? '',
            'server_ip'       => end($serverIpTemp),
            'server_name'     => $serverIdentifies,
            // 手动获取北京时区时间戳
            'notice_time'     => app()->datetime->cn->format('Y-m-d H:i:s'),
            'content'         => ['info' => $info],
            self::ERROR_LEVEL => $errorInfo[ self::ERROR_LEVEL ] ?? '',
            'is_test'         => $isTest ? 1 : 0,
        ];
