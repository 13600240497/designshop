...
export default {
    data () {
        return {
            ...
            cateid: '',
            current_cateid: '',
            lasttime: this.$root.data.last_save_time || new Date().getTime(),
            loop_hour: this.$root.data.loop_hour || ''
        };
    },
    computed: {
        is_whole () {
    ...
    methods: {
        ...
        // 根据循环时间获取对应的ID
        get_current_id_by_hour () {
            if (this.loop_hour != '') {
                let now = new Date().getTime();
                let hour = (now - this.lasttime) / 1000 / 3600;
                let array_index = parseInt(((hour / this.loop_hour) % this.cateid.length));
                return array_index;
            } else {
                return 0;
            };
    ...
    created () {
        ...
        this.cateid = this.$root.data.cateId != null ? this.$root.data.cateId.toString().split(',') : '';
    },
    mounted () {
        // 有循环小时的
        const index = this.get_current_id_by_hour();
        this.current_cateid = this.cateid[index] || '';
        this.get_data();
    }
};
</script>
