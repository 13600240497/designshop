...
(function() {
	...
	/* 每隔四个小时切换一次商品 */
	function turnGoods() {
		$('[data-gid=U000031_promotions]').each(function(index, element) {
	...
	/*多条促销信息显示*/
	$('.gs-off-more').on("click", function() {
		var $first_text = $(this).find('.gs-off-text');
		if ( $first_text.length > 1 ) {
  			$(this).addClass('line-border');
		}

	})
	$('.active-close').on("click", function(event){
		event.stopPropagation();
		$(this).parents('.gs-off-more').removeClass('line-border')
	})

	/* 价格溢出，跨行显示 */
	$('.goods-item-bigPromote .my_shop_block').each( function() {
		var $this = $(this);
		var $shop_price_parents = $this.parents('.gs-goods-price').width();
		if( $this.width() > $shop_price_parents/2 ) {
			$('.my_shop_block').addClass('text-over-price');
		}
