setTimeout(function () {
	...
	for (let i = 0; i < timeDateLenth; i++) {
		...
		if (timeDate.toString() != "") {
			if (imgIndex !== false) {
				$(".js_geshopTimeZoneBanner").eq(i).prev(".initImgSrc").css("background-image", "url(" + imgsrcList[imgIndex] + ")");
				//   .css('backgroundImage', imgsrcList[imgIndex]);
				// $(".initImgSrc").attr('src', imgsrcList[imgIndex])
			} else {
				if (imgsrcList[0] != "") {
					$(".js_geshopTimeZoneBanner").eq(i).prev(".initImgSrc").css("background-image", "url(" + imgsrcList[0] + ")");
					// .attr('src', imgsrcList[0]);
