...
if (!GS_spikeGoodsTab) {
	...
	/* tab切换 */
	$('[data-gid="U000078_tabspike"] .gs-goods-tab').on('click', '.gs-goods-bd li', function () {
		...
		// $('[data-gid="U000078_tabspike"] .gs-goods-tab').find('.gs-tab_going').css({ background: tabColorInput.data('backgroundColor'), color: tabColorInput.data('color'), borderColor: tabColorInput.data('borderColor') })
		// $('[data-gid="U000078_tabspike"] .gs-goods-tab').find('.gs-tab_going .gs_component_countDown').css({ color: tabColorInput.data('color') })
	...
	if (!gs_spikeKillGlobal) {
		var gs_spikeKillGlobal = function (my) {
			var $gsCountTarget = $('[data-gid="U000078_tabspike"] .gs-goods-tab .gs_component_countDown');
			my.initCountDown = function () {
				$gsCountTarget = $('[data-gid="U000078_tabspike"] .gs-goods-tab .gs_component_countDown');
				$gsCountTarget.each(function () {
					...
					if ($self.closest('.gs-tab-title-item')) {
						...
						if (dataStartTime) {
							...
							$tabTarget.find('.gs-goods-date_hours').text(GS_spikeGoodsTab.FormatHour(dataStartTime))
				...
				/* 当前活动或即将开始 */
				$('[data-gid="U000078_tabspike"] .gs-goods-tab').each(function (i,element) {
					var tabSelect = false;
			...
			my.compoentCountDown = function () {
				...
				$gsCountTarget.each(function () {
					...
					// var $wrapper = $self.parents('[data-gid="U000078_tabspike"]:eq(0)');
	...
	function tabSpikeScrollEvent () {
		var $goodTarget = $('[data-gid="U000078_tabspike"] .gs-goods-tab');
	...
	var isEditEnv = $('[data-editenv]:eq(0)').data('editenv');
	if (isEditEnv == '1') {
		$('[data-gid="U000078_tabspike"] .gs-tab-content').find('.gs-tab-item:eq(0)').addClass('gs-tab-show').siblings().removeClass('gs-tab-show');
	}
	tabSpikeScrollEvent();
}
});
