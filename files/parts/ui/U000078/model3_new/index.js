$(function() {
	...
	function turnGoods() {
		$('[data-gid=U000078_zaful]').each(function(index, element) {

			/* 每隔四个小时切换一次商品 */
			if (window.localStorage) {
				var nowTime = new Date().getTime();
				var $fixed_goods_count = parseInt($(element).find('.gs-goods-singleTab').attr('data-fixed-g-count'));
				var $default_goods_count = parseInt($(element).find('.gs-goods-singleTab').attr('data-default-g-count'));
	...
	function showGoodsRandom(e) {
		var $default_goods_count = parseInt($(e).find('.gs-goods-singleTab').attr('data-default-g-count'));
		var $fixed_goods_count = parseInt($(e).find('.gs-goods-singleTab').attr('data-fixed-g-count'));
		var $key = $(e).attr('data-key');
		var $id = $(e).attr('data-id');
		var $total = $(e).find('.gs-goods-singleTab').attr('data-skus') != "" ? parseInt($(e).find('.gs-goods-singleTab').attr('data-skus').split(',').length) : "";
	...
	function  default_time_sku(e) {
		...
		var $key = $(e).attr('data-key');
		var $id = $(e).attr('data-id');
		var $total = parseInt($(e).find('.gs-goods-singleTab').attr('data-skus').split(',').length);
		var init_time = parseInt($(e).find('.gs-goods-singleTab').attr('data-init-time'));
...
if (!GS_goods_kill) {
	...
	if (!GS_goods_kill) {
		var GS_goods_kill = function (my) {
			...
			my.tabScrollEvent = function () {
				var $goodTarget = $('.gs-goods-singleTab');
	...
	function tplIntCallback (target) {
		...
		GS_goods_kill.getTplProduct(currentSkus, target).done(function (res) {
			if (isEditEnv == '1') {
				Design != undefined && Design.disableLoading()
				Design != undefined && Design.disableLayuiLoading()
			}
			...
			if (res.code == '0' && dataList) {
				...
				gs_laytpl(tplHtml).render(dataList, function (html) {
					if (html) {
						...
						$(element).find('.gs-btn-area .gs-btn').each(function(){
							if($(this).height() > 35){
								$(this).css({'line-height':'22px','padding-top':'5px','padding-bottom':'5px'});
							}
						});
					}
				})
