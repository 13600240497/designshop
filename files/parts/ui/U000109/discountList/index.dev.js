$(function () {
	var tabBox= $('[data-gid="U000109_discountList"] .gs-tab-box');
 ...
 var staticDomain = $('[data-static-domain]:eq(0)').attr('data-static-domain') ? $('[data-static-domain]:eq(0)').attr('data-static-domain') : "";
  ...
  var LBTplObj = {
    renderTpl: function () {
      ...
      // 遍历 - 兼容多个组件各自获取数据
      $('.geshop-leaderboard-async-list[data-gid="U000109_discountList"]').each(function(i, element) {
        var $ele = $(element);
        ...
        function scrollCalBackFn($targetBox,scrollTop){
            $targetBox.each(function(){
                ...
                if (gsTabOffset.top <= scrollTop && scrollTop < gsTabOffset.top + $this.find('.geshop-content').outerHeight()) {
                  $this.find(".gs-tab-wrap-fixbox").css({"position":"fixed",top:0,left:gsTabOffset.left+'px',"width":gsTabBoxWidth+'px','z-index':9999});
                  $this.addClass('js-geshop-nav-fixed');

                  //$('#nav-list').closest('[class*="fix"]').hide();
