...
$(function() {
    var components = $('.geshop-U000195-default');
    ...
    this.init = {
        ...
        check_my_ranking: function (className) {
            ...
            this.get_ajax_data(url, params, 'check').done(function (res) { 
                ...
                if (res.code == 0) {
                    var today_shop_price_html = '<span class="amount my_shop_price" data-orgp="' + res.data.todayTotal + '">' + res.data.todayTotal + '</span>';
                    var total_shop_price_html = '<span class="amount my_shop_price" data-orgp="' + res.data.areaTotal + '">' + res.data.areaTotal + '</span>';
                    var rank = res.data.rank >= 100 ? '100+' : res.data.rank;
                    var rank_html = '<span class="amount">' + _this.languages.myRankingNum.replace('xxx', rank) + '</span>';
                    var html = '<p class="ranking-item">' + _this.languages.todayAmount.replace('xxx.xx', today_shop_price_html) + '</p>'+
                        '<p class="ranking-item">' + _this.languages.totalAmount.replace('xxx.xx', total_shop_price_html) + '</p>'+
                        '<p class="ranking-item">' + _this.languages.myRanking.replace('xxx', rank_html) + '</p>';

                    $(elem).find('.ge-195d-dialog-my-ranking .content').html(html);
                    typeof GEShopSiteCommon !== 'undefined' && GEShopSiteCommon.renderCurrency();
                    _this.show_dialog(className);
                } 
                // code 100 
                // 接口返回错误状态码都是100，前端无法判断，先根据message判断
                    ...
                    var loginUrl = (typeof HTTPS_LOGIN_DOMAIN !== 'undefined' ? HTTPS_LOGIN_DOMAIN : (DOMAIN_LOGIN + '/')) + GESHOP_LANG + '/m-users-a-sign.htm';
