...
function judge_show () {
    // 判断是否有缓存
    if (localStorage['U000257-template1']) {
        // 上次的时间戳
        const last_timestamp = new Date(parseInt(localStorage['U000257-template1']));
...
function update_show_time () {
    localStorage['U000257-template1'] = new Date().getTime();
}

/**
 * 打开弹窗
 */
function show_dialog_257_tempalte1 () {
    var container = $('.U000257-template1');
        container.addClass('is-fixed');
    update_show_time();
}

function close_dialog_257_template1 () {
    var container = $('.U000257-template1');
...
!(function() {
     ...
     */
    window.info_check && window.info_check().done(function (res) {
        // 已经登录 + 非当天注册
        if (res.user_id != 0 && res.is_today_reg == false) {
            // 判断缓存是否出现过
            judge_show() && show_dialog_257_tempalte1();
        }
    });
     ...
     * 遍历所有组件
     */
    $('.U000257-template1').each(function() {
        var $component = $(this);
