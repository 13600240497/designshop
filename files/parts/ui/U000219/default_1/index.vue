<template>
    <div :class="style_class" :style="style_body" :data-id="pid">
...
export default {
    ...
    mounted () {
        ...
        if (this.data.goodsInfo && this.data.goodsInfo.length > 0) {
            ...
            this.arrData = this.data.goodsInfo;

            // 默认商品个数是否为空
            if (this.show_goods_number !== '') {
                // 小于所有商品总数时，出现view more
                if (this.show_goods_number < this.data.goodsInfo.length) {
                    this.isMore = true;
                    this.list = this.data.goodsInfo.slice(0, this.show_goods_number);
                }
            }
        } else {
