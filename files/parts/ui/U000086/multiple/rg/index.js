(function () {
    $.extend($.fn, {
        U000086: function () {
            ...
            var endedText = _getAttr('endedText');
            var pageInstanceId = _getAttr('pageInstanceId');
            var couponData = window['couponData' + pageInstanceId];
            var length = couponData.length;
            var currentTimestamp = new Date().getTime()
            var couponId = -1;
            var couponIndex = -1;

            for (var index = 0; index < length; index = index + 1) {
                if (currentTimestamp >= couponData[index].startTimestamp) {
            ...
            if (couponId == -1) {
                ...
                couponIndex = 0;
            } else if (couponIndex < length - 1 && couponData[couponIndex].endTimestamp <= currentTimestamp) {
                couponId = couponData[couponIndex + 1].id;
            ...
            var _diffTime = parseInt(serverTime) - currentTimestamp;
            //如果是1的话就是编辑环境
            ...
            $.extend(CountDown.prototype, {
                changeCouponStatus: function (flag) {
                    ...
                    if(status){
                        ...
                        this.$node.find('.coupon-text').html(couponData[couponIndex].text);
                    }
                },
                ...
                bindEvent: function(){
                    ...
                    if (__dev) return;
                    var _this = this;
                    this.$node.on('click', '.coupon-bottom .coupon-btn', function () {
                        //如果是未开始的时候点击
                        if (_this.status === 1) {
                        ...
                        // Has ended
                        if (_this.status === 4) {
                            GEShopSiteCommon.dialog.message(GESHOP_LANGUAGES.coupon_end_msg)
                            return;
                        }
                ...
                getCouponDetail: function(){
                    ...
                    window.GEShopCommonFn_Vue.$jsonp(url, params, { pid: pid }).done(function (res) {
                        if (res.code === 0) {
                                ...
                                if (siteCode == 'rg') {
                                    endTime = parseInt(res.data.couponInfo.getEndTime) * 1000;
                                } else {
                                    endTime = parseInt(res.data.couponInfo.enableEndTime) * 1000;
                                }

                                if (endTime <= new Date().getTime()) {
                                    _this.setDiscount(couponInfo.offerAmount, couponInfo.thresholdAmount, couponInfo.type);
                                    _this.changeCouponStatus('4')
                ...
                setDiscount: function (n1, n2, type) {
                    ...
                    if (parseInt(type) === 1) {
                        ...
                        this.$node.find('.coupon-discount-term i').attr('data-orgp',n2).html('$' + n2);
                    } else if (parseInt(type) == 2) {
                        this.$node.find('.coupon-discount').attr('data-orgp',n1).html(sign + '$' + n1);
                        if (n2 == 0) {
                            this.$node.find('.coupon-discount-term i').hide();
                        } else {
                            this.$node.find('.coupon-discount-term i').attr('data-orgp',n2).html('$' + n2);
                        }
                    }
    ...
    $(function () {
        $('.geshop-u000086-multiple').each(function (index, el) {
            $(el).U000086();
        });
    });
}());
