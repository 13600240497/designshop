<template>
    <div class="geshop-u000166-coupon2" v-show="layOutShow">
        <div class="u000166-coupon2-box">
           <div class="coupon2-info-group">
               <!--库存条-->
               <div class="good-range-box" v-if="data.coupon_range == 0">
                   <span v-if="data.coupon_range_show == 0">{{ goodsPercent }}% {{ this.languages.pcs_left[this.lang] }}</span>
        ...
        <div class="dia-coupon-getting" v-show="gettingShow">
            <div class="getting-main">
                <div class="getting-info">
                    ...
                    <!--<span class="btn-later" @click="handleShowGetDia(false)">{{ this.languages.later[this.lang] }}</span>-->
                </div>
...
<script>
import languages from '../../U000164/coupon2/language';
export default {
    props: ['pid', 'data'],
    data () {
        return {
            ...
            couponInfo: {
                ...
                leftCount: 2, // 优惠券剩余个数
                maxCount: 10, // 优惠券总个数
                site_url: '//m.wap-rosegal.com.master.php5.egomsl.com', // 站点首页
                coupon_url: '//userm.wap-rosegal.com.master.php5.egomsl.com/m-users-a-coupon.html' // 个人优惠券中心
            },
            languages, // 多语言翻译
            score: '您确定使用<span>200</span>积分兑换优惠券吗？', //  您确定使用{score}积分兑换优惠券吗？ score 替换
    ...
    methods: {
        ...
        /**
         * 兑换弹窗
         * @param {boolean} boolean : boolean 为 true 显示， false 关闭
         *
         */
        handleShowGetDia (boolean) {
            if (!this.data.active_id) { // 没有填写id 返回
                return false;
            }
         ...
         *
         */
        handleShowTipsDia (boolean) {
            this.tipsShow = boolean;
        },
        // 获取优惠券
        getCoupon () {
