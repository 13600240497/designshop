$(function () {
    $.extend($.fn,{
        U0001652 : function(){
            ...
            function _jsonp (param) {
                param = param || {};
                var ext = $box.parents('.geshop-component-box').eq(0).attr('data-id');
                var jsonpCallback = `geshop_callback_${ext}_` + param.function_name
                return $.ajax($.extend({
                    cache: true,
                    dataType: "jsonp",
                    jsonp: 'callback',
            ...
            $.extend(CountDown.prototype,{
                ...
                bindEvent : function(){
                    ...
                    this.$node.on('click',function(){
                        //如果是未开始的时候点击
                        if(_this.status === 1){
                            GEShopSiteCommon.dialog.message('Coupons are not available until the event begins');
                            return;
                        }
                    ...
                    _jsonp({
                        ...
                        success :function(res){
                            if(res.code === 0){
                                ...
                                if(couponInfo.offerCurrency && window.my_array_sign){
    ...
    $('[data-module="U000165-2"]').each(function(index,el){
        $(el).U0001652();
    })
});
