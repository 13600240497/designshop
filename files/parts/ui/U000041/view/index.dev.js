;$(function () {
    var tab_item = $('.wrap-U000041_view').find('.gs-tab-box li');
    ...
    /* 每隔四个小时切换一次商品 */
    function turnGoods() {
        $('[data-gid="U000041_view"]').each(function (index, element) {
    ...
    $(".gs-tab-item").each(function (i, ele) {
        var view = $(ele).find(".quick_view");
        view.on('click', function (e) {
            e.stopPropagation();
            var sites = $(this).attr('data-site').split("-")[0];
            // var goods_sn = $(this).attr('data-goodsSn');
            var goods_id = $(this).attr('data-goodsId');
            var siteDomain = $(this).attr("data-siteDomain");
            var url = "";
            switch (sites) {
                case 'rw':
    ...
    function setFixed(top) {
        ...
        $.each(components, function (i, item) {
            if (item.tabWrap.data('tab-fix') == 1) {
                ...
                if (top + 2 >= item.tabWrap.offset().top && item.tabWrap.offset().top + item.tabWrap.outerHeight(true) >= item.tabHeight + top) {
                    item.tab.css({
                        position: 'fixed',
    ...
    // 		var top = site == 'zf' ? item.content.offset().top : item.content.offset().top - item.tab.outerHeight(true);

    // 		if (t > top && t < (top + item.tabHeight + item.contentHeight)) {
    // 			item.tab.css({
    // 				position:"fixed",
    // 				height:item.tab.height(),
    // 				width:item.tab.width(),
    // 				left:item.left,
    // 				top: site == "zf" ? ($('#top.active').length > 0 ? $('#top.active').outerHeight() : 'auto') : 0,
    // 				"z-index":10,
    // 				overflow:"hidden"
    // 			})
    // 		}else{
