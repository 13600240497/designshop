$(function() {
  ...
  $('.geshop-U000072-fixedbottom').each(function(){
    var $component = $(this);
    ...
    $('body').on("click",function(e){
      ...
      if(editEnv == 0){
        ...
        if(element && !$(element).hasClass("deals-main-container") && !$(element).hasClass('swiper-pagination-bullet') && !$(element).hasClass("swiper-pagination") && !$(element).hasClass('swiper-container') && !$(element).hasClass("swiper-img") && !$(element).hasClass("center-align") && !$(element).hasClass("deal-button") && !$(element).hasClass("img-fold")){
          var foldSrc = $('.main-container').find('.img-fold').attr('data-fold-src');
          $('.main-container').find('.img-fold').attr('src',foldSrc);
          $('.main-container').removeClass('open');
          $('.overlay').removeClass('open');
