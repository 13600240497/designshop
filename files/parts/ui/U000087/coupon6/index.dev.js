;$(function() {
    ...
    $LAB.script(staticDomain + "/resources/javascripts/library/gs_laytpl.js?2018100101").wait(function () {
        ...
        /// 获取值
        $('.geshop-U000087-coupon6').each(function(index, elem){
            var component = $(elem);
        ...
        // 批量发送AJAX获取数据，等待 appLogin 之后再执行
        var coupunParam = {
            lang: GESHOP_LANG || 'en',
        ...
        $.ajax({
            ...
            dataType: 'jsonp',
            jsonpCallback: 'geshop_callback_087_coupon6_list',
    ...
    function gsInitCouponById(params, index) {
        ...
        function gsCounponRender(data) {
            ...
            if (data.receivedCount >= 1) {
                ...
                if (data.resource == 0) {
                    ...
                    if (data.type === 1) {
                        // M端或APP返回链接，配置链接或者首页
                        use_link = getSiteUrlByPlatform(percentAllCategoryLink)
                    } else {
                        // 非百分比的全站类型，跳首页
                        use_link = data.site_url
                    }
                }
                if (data.resource == 1) {
        ...
        // M或者APP链接转换
        function getSiteUrlByPlatform(url) {
            if (GESHOP_PLATFORM === 'app') {
                return 'rosegal://action?actiontype=0'
            } else {
                return url
            }
        }
        ...
        function gsCounponBindEvents(elem, data) {
            ...
            elem.on('click', '[data-event="getCoupon"]', function() {
                $.ajax({
                    // url: 'http://m.wap-rosegal.com.geshop.php5.egomsl.com/geshop/goods/getcoupon',
                        ...
                        if (res.code === 0) {
                            ...
                            if (data.resource == 0) {
                                ...
                                if (data.type === 1) {
                                    ...
                                    // 非百分比的全站类型，跳首页
                                    use_link = res.data.site_url
                                }
                            }
                            if (data.resource == 1) use_link = data.category_url;
                            if (data.resource == 2) use_link = data.product_link;
                            dom.find('.gc4-get-btn[data-type="canUse"]').attr('data-url', use_link)
                        } else if (res.code === 1) {
                            // {# 获取优惠券未登陆状态 跳转登录页面 #}
                            var login_url = GESHOP_PLATFORM === 'app' ? 'webAction://checkUserInfo?callback=appUserInfo()&isAlert=1' : res.data.loginurl;
                            GEShopSiteCommon.webLoginAction(login_url);
                        } else if (res.code === 5) {
