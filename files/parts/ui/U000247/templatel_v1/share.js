(function () {
    ...
    window.U000247_shareSuccess = function ({ url, params, id }, cb) {
        $.ajax({
            url: url,
            type: 'GET',
            cache: true,
            ...
            data: params,
            timeout: 3000,
            jsonpCallback: `geshop_callback_${id}`,
    ...
    window.U000247_app_shareSuccess = function (msg, type, cb) {
        if (msg == 1) {
            let data = Object.assign({}, GESHOP_STORE.state.dresslily.lott_params, { share_channel: type });
            $.ajax({
                url: GESHOP_INTERFACE.elf_webgame_share_prize.url,
                type: 'GET',
                cache: true,
                dataType: 'jsonp',
                data: data,
                timeout: 3000,
                jsonpCallback: `geshop_callback_${GESHOP_STORE.state.dresslily.lott_params.id}`,
                success: function (data) {
                    if (typeof cb === 'function') {
                        cb(data);
                    }
                    if (data.status == 200) {
                        GESHOP_STORE.commit('dresslily/set_left_time', 1);
                    }
                },
                error: function () {
                    if (typeof err == 'function') {
