$(function () {
    ...
    function successCb (cartNum) {
        ...
        if (GESHOP_SITECODE == 'zf-wap' || GESHOP_SITECODE == 'zf-app') {
            var $cart_target = $('.js_cart_nums');
            var originalCartNum = $cart_target.text() == '' ? 0 : parseInt($cart_target.text());
            var totalNum = originalCartNum + parseInt(cartNum.val());
            if (totalNum > 99) {
                $cart_target.text('99+');
            } else {
                $cart_target.text(totalNum);
            }
            totalNum > 0 && $cart_target.show();
            layer.open({
     ...
     */
    function canAddCart () {
        $('.geshop-U000192-default').each(function () {
        ...
        $('.geshop-U000192-default').each(function () {
            $(this).find('.goods-list li').each(function () {
                ...
                if (unique(colorAttr).length > 1) {
                    ...
                    $(this).find('.js-color-btn').addClass('noneArrow');
                }
                ...
                if (unique(sizeAttr).length > 1) {
                    ...
                    $(this).find('.js-size-btn').addClass('noneArrow');
                }
    ...
    if (isEditenv != 1) {
        $LAB.script(GESHOP_STATIC + '/resources/javascripts/library/in-view.js').wait(function () {
            $('.geshop-U000192-default').each(function (i, v) {
                inView('.geshop-U000192-default-' + $(v).data('id')).once('enter', function (el) {
                    renderHtml(el);
                });
            })
        })
    } else {
        renderHtml($('.geshop-U000192-default'));
    }
            ...
            window.GEShopCommonFn_Vue.$jsonp(url, params, { pid: pid }).done(function (res) {
                if (res.code == '0') {
                    for (var i = 0; i < res.data.length; i++) {
                        var component = isEditenv == 1 ? $('.geshop-U000192-default').eq(i) : $(el);
                        var html;
                        ...
                        if (res.data[i].goods_list.length == 0) {
                            ...
                            html += '<img src="https://geshopimg.logsss.com/uploads/FRcjv8nNOT0BbSfV9ht3ZLemuo1K2AqQ.png" class="js_gdexp_lazy js-geshopImg-lazyload" data-original="https://geshopimg.logsss.com/uploads/FRcjv8nNOT0BbSfV9ht3ZLemuo1K2AqQ.png" alt="zaful" title="zaful" style="display: inline;">';
                            html += '</a>';
                            ...
                            for (var key in res.data[i].goods_list) {
                                if (res.data[i].goods_list.hasOwnProperty(skus[index])) {
                                    ...
                                    html += '<img src="https://geshopimg.logsss.com/uploads/FRcjv8nNOT0BbSfV9ht3ZLemuo1K2AqQ.png"  data-original="' + item.goods_img + '" class="js_gdexp_lazy js-geshopImg-lazyload" style="display: inline;">';
                                    html += '</a>';
                                    html += '	<p class="mt10 price-wrap">';
                                    html += '<span class="my_shop_price price js-shop-price site-bold-strict" data-orgp="' + item.shop_price + '" data-bz="' + item.shop_price + '">' + item.shop_price + '</span>';
                                    html += '</p>';
                                    html += '<div class="mt10">';
                                    html += '<span class="attribute-wrap pr">';
                            ...
                            if (item.active_num == 0 || item.is_on_sale == 0) {
                                html += '<div class="cart-btn-wrap js-add-cart btn__site-default disabled">';
                            } else {
                                html += '<div class="cart-btn-wrap js-add-cart btn__site-default">';
                            }
                            html += '<i class="ico-cart"></i>';
