<template>
    <div class="geshop-U000182-template2_v2-body"
         :style="style_body"
...
export default {
    ...
    methods: {
        init () {
            const _this = this;
            try {
                let listData = this.goodsInfo;
                this.list = listData.length > 0 ? listData : (window.GESHOP_PAGE_TYPE == 1 ? this.list : []);
            } catch (e) {
            ...
            if (!this.list && this.left_active[this.platform] == '0') {
                this.list = [
                    { 1: 0 },
                    { 1: 0 },
                    { 1: 0 },
            ...
            that.$nextTick(() => {
                ...
                if (window.GS_GOODS_LAZY_FN) {
                    window.GS_GOODS_LAZY_FN($('.geshop-U000182-template2_v2-body img.js_gdexp_lazy'));
                }
            });
        },
        debounce (fn, delay) {
            ...
            this.$nextTick(() => {
                ...
                if (platfrom === 'pc') {
                    ...
                    this.mySwiper = new Swiper(this.$el.querySelector('.swiper-container'), {
                        ...
                        on: {
                            init: function (swiper) {
                                ...
                                if (this.list && slideNum == this.list.length) {
                    ...
                    if (platfrom === 'pad') {
                        ...
                        // slidesPerGroup = 3;
                        if (this.list && this.list.length > slideNum) {
                            loopState = true;
                        } else {
                            this.mNavShow = false;
                        }
...
<style lang="less" scoped>
    .geshop-U000182-template2_v2-body {
