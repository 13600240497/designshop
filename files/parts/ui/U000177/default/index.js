...
(function () {
	...
	var LBTplObj = {
		renderTpl: function () {
			...
			// 遍历 - 兼容多个组件各自获取数据
			$('.wrap-U000177-default').each(function (i, element) {
				var $ele = $(element);
				    ...
				    window.GEShopCommonFn_Vue.$jsonp(url,params,{target:$(this)}).done(function(res){
                        ...
                        gs_laytpl(getTpl).render(data, function (html) {
                            ...
                            if (window.GESHOP_UTIL && typeof window.GESHOP_UTIL.goodsLazy === 'function') {
                                window.GESHOP_UTIL.goodsLazy($('.js-leader-board-container-U000177:eq('+i+')').find('.js-geshopImg-lazyload'));
                            }
                        });
                    });
				}
   ...
   */
	function renderSwiperFn (element) {
		...
		var swiper = new Swiper3(swiperContainer, {
