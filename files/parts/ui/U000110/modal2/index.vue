<template>
    <div class="geshop-u000110-modal2"
         :style="{
            backgroundColor : data.box_bg_color || '#F2F2F2',
            marginBottom: `${ data.box_margin_bottom / 75 || 40/75}rem`
          }">
        <img :src="data.bannerImg || 'https://geshopimg.logsss.com/uploads/XVbaglsB9icvKSeMTxdC2PFY4WQ8tyuA.png'" alt="banner" class="top-banner" v-if="data.banner_show == 1" :name="data.banner_show">
        <div class="list-wrap" ref="swiper2">
            <div class="swiper-container" :style="{backgroundColor: data.tabBgc || '#ffffff' , 'border-radius': `${data.tabBorderRadius ? data.tabBorderRadius / 75 : (12 / 75) }rem`}">
                        ...
                        <li class="list-item" v-for="(item, index) in goodInfo" :key="item.goods_sn">
                            ...
                            <div class="item-info-box">
                                ...
                                <P class="sale-discount" :style="{ backgroundColor: data.discount_bg_color, color: data.discount_font_color }">
                                    <span>{{ $lang("hotsale_rank_label").replace('XX', item.sale_number)}}</span>
                                </P>
                            ...
                            <span class="btn-rank mid"
                                ...
                                <span :style="{'font-size': index + 1 > 99 ? 18/75+'rem' : '', color: `${data.iconOtherTextColor || '#ffffff'}`}">{{ index + 1}}</span>
                            </span>
                            <geshop-analytics-href
                                    :href="item.url_title"
                                    :sku="item.goods_sn"
                ...
                <div
                    ...
                    <ul>
                        <li class="list-item" v-for="(item, index) in 10" :key="index">
                            ...
                            <div class="item-info-box">
                                <a href="#" class="item-title">ZAFUL{{cur}} Plunge Slit Printed Maxi Dress - Red Wine M</a>
                                <P class="sale-discount" :style="{ backgroundColor: data.discount_bg_color || '#ffeecc', color: data.discount_font_color || '#ffa800' }">
                                    <span>8448</span><span>{{ $root.languages.sold }}</span>
                                </P>
                                <p class="item-shop-price">
...
export default {
    ...
    created () {
        ...
        $LAB.script(GESHOP_STATIC + '/resources/javascripts/library/swiper/swiper.3.4.spec.min.js').wait(() => {
            ...
            this.$nextTick(() => {
                this.init();
                this.$boxWrap = $(this.$refs.swiper2);
            });
        });
    },
    methods: {
        async init () {
            ...
            if (this.data.goodsIds && this.data.goodsIds.length && this.data.goodsIds[0].cateid) {
                this.getGoods(0);
            }
            this.bindScroll();
        },
