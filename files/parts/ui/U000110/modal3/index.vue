<template>
    <div class="geshop-u000110-modal3"
         :style="{
        ...
        <div class="list-wrap" ref="swiper3">
            ...
            <div class="list-group">
                <div class="cont-list"
                     v-if="data.goodsIds != '' && data.goodsIds[0].cateid != '' && index == cur"
                     v-for="(items, index) in data.goodsIds"
                     :key="items.cateid">
                    <ul>
                        <li class="list-item" v-for="(item, index) in goodInfo" :key="item.goods_sn">
                            ...
                            <div class="item-info-box">
                                ...
                                <P class="sale-discount"
                                   :style="{ backgroundColor: data.discount_bg_color, color: data.discount_font_color }">
                                    <span>{{ $lang("new_rank_label").replace('XX', item.sale_number).replace('XX', item.sale_days)}}</span>
                                </P>
                ...
                <div class="cont-box" :data-name="index"
                    ...
                    <ul>
                        <li class="list-item" v-for="(item, index) in 10" :key="index">
                            ...
                            <div class="item-info-box">
                                <a href="#" class="item-title">ZAFUL{{cur}} Plunge Slit Printed Maxi Dress - Red
                                    Wine</a>
                                <P class="sale-discount"
                                   :style="{ backgroundColor: data.discount_bg_color || '#ffeecc', color: data.discount_font_color || '#ffa800' }">
                                    <span>9999 {{ $root.languages.sold.replace('XX', 99) }}</span>
                                </P>
                                <p class="item-shop-price">
...
export default {
    ...
    created () {
        ...
        $LAB.script(GESHOP_STATIC + '/resources/javascripts/library/swiper/swiper.3.4.spec.min.js').wait(() => {
            ...
            this.$nextTick(() => {
                this.init();
                this.$boxWrap = $(this.$refs.swiper3);
            });
        });
    },
    methods: {
        ...
        async getGoods (index) {
            ...
            if (_this.data.goodsIds[index].cateid) {
                try {
                    ...
                    this.goodsData[index] = res.data.goodsInfo;
                    // 页面元素初始化
                    this.$store.dispatch('global/async_goods_init', this);
                } catch (err) {}
            }
