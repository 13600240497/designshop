$(function () {
    var tab_item = $('[data-gid="U000110_newProductList"] .gs-tab-box li');

    $('[data-gid="U000110_newProductList"] .gs-tab-box').each(function () {
        var $box = $(this);
        var ulW = $box.find('.goods-nav-name > li').length;
    ...
    var $hotSaleList = $('.wrap-U000110_newProductList');
...
$(function () {
    ...
    var LBTplObj = {
        renderTpl: function () {
            ...
            // 遍历 - 兼容多个组件各自获取数据
            $('.geshop-leaderboard-async-list[data-gid="U000110_newProductList"]').each(function (i, element) {
                var $ele = $(element);
                ...
                /* 判断读取正式数据或者测试数据 */
                if (params.cateid != null) {
                    loadPage(params, 'current', _listRender, component_id);
                } else {
                    _listRender({ goodsInfo: [] });
                }
                ...
                $ele.on('click', '.layui-laypage-prev, .layui-laypage-next', function () {
                    ...
                    loadPage(params, _act, _listRender, component_id)
                });
    ...
    @callback 渲染回调方法
  */
    function loadPage(params, action, callback, pid) {
        ...
        $.ajax({
            ...
            cache: true,
            jsonpCallback: 'geshop_callback_' + pid,
