...
function MultiDataRange(el, dataSrouce) {
    ...
    this.bgColor = dataSrouce.bgColor || "#333333";
    ...
    this.render = function () {
        ...
        var hour = this.numberToString(arr[0]);
        var minute = this.numberToString(arr[1]);
        var second = this.numberToString(arr[2]);
        this.dom.find('.multi-dateRange-numbers label').eq(0).html(hour);
        this.dom.find('.multi-dateRange-numbers label').eq(1).html(minute);
        this.dom.find('.multi-dateRange-numbers label').eq(2).html(second);
        ...
        if (this.isEditEnv == true) {
            ...
            if (this.status != 1) this.dom.remove();
            if (this.status == 1 || this.dom.index() == 5)  this.dom.addClass('active');
        }
