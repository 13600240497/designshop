$(function () {
  ...
  $(window).scroll(function () {
    ...
    if ($(this).scrollTop() >=  navigateTarget.parent().offset().top - GESHOP_UTIL.getSiteFiedHeader()) {
        nav_is_fixed = true;
        navigateTarget.addClass('component-nav-fixed').css('top', GESHOP_UTIL.getSiteFiedHeader());
        // 站点导航栏处理
        ...
        nav_is_fixed = false;
        navigateTarget.removeClass('component-nav-fixed').css('top', 'auto');
        if ($('.js-geshop-nav').length) {
            $('.js-geshop-nav').show();
        }
    }
    ...
    timer = setTimeout(function () {
        ...
        if (scrollTopBefore == scrollTopAfter) {
            ...
            if (scrollTopAfter >= parseInt(titleTarget.offset().top) - parseInt(navigateTarget.height()) - $('#top.active').outerHeight()) {
  ...
  navigateTarget.on('click', 'li', function () {
    ...
    // $('html, body').animate({
    //   scrollTop: $('.geshop-component-box[data-id=' + navigatedId + ']').offset().top - navigateTarget.outerHeight() - $('#top').outerHeight()
    // }, 500);
