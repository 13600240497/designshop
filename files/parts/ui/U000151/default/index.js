...
(function () {
    ...
    $.extend($.fn,{
        ...
        U000151 : function(element){
            ...
            var defaultImage = $hidden.hiddenAttr('defaultImage');//默认图片路劲
            ...
            var _addCountDown1 = function _addCountDown1($li,item){
                ...
                var activity_volume_number = parseInt(item.activity_volume_number);
                if(activity_volume_number >= activity_number){
                    $li.find('.act-sale-out').show();
                }
            }
            //初始化渲染列表内容
            /**
            ...
            var _initGoods = function _initGoods(index,list,act){
                ...
                for( var i = 0 ; i < list.length; i++ ){
                    list[i].url_title = geshopUrlToApp(list[i].url_title, list[i].goods_id);
                    _arr.push(_substitute(_tmp,$.extend({goods_img : defaultImage,index : i + 1},list[i],act)))
                }
                var $ul = $tabItem.eq(index);
                $tabItem.eq(index).find('ul').html(_arr.join(''));
                //如果活动信息有的话
                if(act.id){
                    //判断是不是卖完的状态
                    ...
                    new CountDown1($.extend({diffTime : _diffTime},act),function(){
                        $ul.find('li').find('.act-sale-out').hide();
            ...
            var _addTab = function _addTab(index,obj){
                ...
                if(!_id){
                    ...
                    window.GEShopCommonFn_Vue.$jsonp(url,params,{target:element}).done(function (res) {
                        res.data.activityInfo = res.data.activityInfo || {};
            ...
            var _moveTo = function _moveTo(n){
                if(n === __index || n <0 || n >= _goodsList.length)return;
    ...
    $('[data-module="U000151"]').each(function(index,el){
        $(el).U000151(el);
    })
})();
