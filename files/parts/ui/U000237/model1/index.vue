<template>
    <div class="geshop-U000237-model1-body" :style="boxStyle" :class="noPreview ? 'component-hide' : ''">
        <ul class="goods_list_wrap">
            <li v-for="(item,index) in dataLists" :key="index" class="clearfix">
                <div class="list_item">
                    ...
                    <div class="item_content">
                        ...
                        <div class="geshop-price-content">
                            ...
                            <div class="item_market_price font_regular">
                                <template
                                    v-if="Number(item.shop_price) < Number(item.market_price) || mockData === true">
...
export default {
    ...
    methods: {
        ...
        showMoreOrLess (type) {
            ...
            if (type == 0) {
                this.isMore = true;
                this.isLess = false;
                this.dataLists = this.dataLists.slice(0, this.data.show_goods_number);
            ...
            if (type == 1) {
                ...
                if (view_more_page < view_max_page) {
                    ...
                    if (page_data) {
                        this.dataLists = this.dataLists.concat(page_data);
    ...
    mounted () {
        ...
        // 是否隐藏组件
        if (data.isEditEnv != 1 && (data.ipsGoodsSKU || data.goodsSKU) && (!data.goodsInfo || (data.goodsInfo && data.goodsInfo.length <= 0))) {
            this.noPreview = true;
        }
        // 获取商品列表数据
        if (data.goodsInfo && data.goodsInfo.length > 0) {
            this.arrData = data.goodsInfo;
            this.mockData = false;
            let goodsArr = data.goodsInfo || [];
            // 默认商品个数是否为空
            if (data.show_goods_number && data.show_goods_number != '') {
                /*            // 小于所有商品总数时，出现view more
                            if (data.show_goods_number < data.goodsInfo.length) {
                                this.isMore = true;
                                this.dataLists = this.dataLists.slice(0, data.show_goods_number);
                            } */
                if (data.show_goods_number < data.goodsInfo.length) {
                    this.isMore = true;
                    this.dataListGroup = this.dataArrayGroup(data.goodsInfo, data.show_goods_number);
                    goodsArr = this.dataListGroup[0];
                    this.view_more_page = 0;
                    this.view_max_page = this.dataListGroup.length > 0 ? this.dataListGroup.length - 1 : 0;
                } else {
                    goodsArr = data.goodsInfo;
                }
            }
            this.dataLists = goodsArr;
        } else {
            this.arrData = this.dataLists;
        }
