...
export default {
    ...
    methods: {
        ...
        getSyncGoodsInfo () {
            let goodsArray = [];
            // 组件唯一ID
            const pidValue = this.pidValue;
            let asyncInfo = this.goodsInfo;
            if (asyncInfo && asyncInfo[pidValue] && asyncInfo[pidValue].length > 0 && asyncInfo[pidValue][0].goodsInfo.length > 0) {
                goodsArray = asyncInfo[pidValue];
            }
            return goodsArray;
        }
    }
