                    ...
                    <div class="model-right">
                        ...
                        <div class="item-price">
                            ...
                            <span class="market-price">
                                <del class="my_shop_price"
                                     :data-orgp="product.market_price">${{ product.market_price }}</del>
                            </span>
                        </div>
                        <div class="select-content select-size" v-if="product.size_list.length > 0">
                            <div class="size-title">{{ $lang('size') }}</div>
                        ...
                        <div class="select-content select-color" v-if="product.color_list.length > 0">
                            <div class="color-title">{{ $lang('color') }}</div>
                        ...
                        <div :class="['add-bag',className]" :data-logsss-event-value="analytics" @click="handleSubmit">
                            {{ $lang('add_to_bag') }}
                        </div>
                    </div>
                </div>
...
export default {
    ...
    computed: {
        ...
        analytics () {
            const params = {
                ...
                ubcta: {
                    rank: this.rank,
                    sku: `${this.product.goods_sn}`,
                    price: `${this.product.shop_price}`,
                    p: this.pid ? `gs-${this.$root.pageId}-${this.pid}` : `gs-${this.$root.pageId}`,
                    fmp: 'mp'
                },
                skuinfo: {
                    sku: this.product.goods_sn,
                    pam: 1,
                    pc: this.product.cateid,
                    k: this.product.warehouse,
                    zt: 0
                }
            };
    ...
    methods: {
        // 展示弹层
        show ({ goods_sn, activity_id, rank }) {
        ...
        async get_product_info () {
            ...
            this.$jsonp(GESHOP_INTERFACE.getlistinspu.url, params).then(res => {
                ...
                if (res.data.goodsInfo.goods_id) {
                    ...
                    GEShopSiteCommon.dialog.message(`<div class="add-fail">${this.$lang('gift_fail_add')}</div>`);
                }
            }, (err) => {
                GEShopSiteCommon.dialog.message(`<div class="add-fail">${err.message || this.$lang('gift_fail_add')}</div>`);
            });
        },
        ...
        handleSubmit () {
            ...
            this.$jsonp(GESHOP_INTERFACE.addgifttocart.url, data).then(res => {
                GEShopSiteCommon.dialog.message(this.$lang('gift_add_cart'));
            }, err => {
