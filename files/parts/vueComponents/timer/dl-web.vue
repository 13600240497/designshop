<template>
    <div class="geshop-dl-timer">
        <div class="geshop-dl-timeBox">
            <!-- 时分秒显示 -->
            <template v-if="format && format === 'hms'">
                <span class="timer-spiner timer-hours-all">{{ hoursContainDay }}</span>
            </template>
            <!-- 天时分秒显示-->
            <template v-else>
...
export default {
    ...
    methods: {
        ...
        updateSt () {
            const cd = {};
            cd[this.pid] = this.status;
            this.$store.commit('dresslily/updateStatus', cd);
        }
    },
    mounted () {
        this.updateSt();
