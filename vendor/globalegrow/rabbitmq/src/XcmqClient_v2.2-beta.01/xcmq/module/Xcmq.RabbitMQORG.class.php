...
class Xcmq_RabbitMQORG extends Xcmq_Abstract
{
    ...
    public function batchPublish($d,$routingkey=null,$exchangeName='amq.direct'){
        ...
        if( $routingkey ){
            if(!isset($this->_routKeyQueue[$routingkey])){
                $return = array();
                foreach($d as $k=>$v){
                    $return[]=array('data'=>$v,'p'=>$this->_p,'code'=>103,'routingkey'=>$routingkey,'exchange'=>$exchangeName,'msg'=>'routingkey:'.$routingkey .' not exsit');
                }
        ...
        if(!in_array($exchangeName,array('amq.default','amq.direct','amq.topic','amq.fanout')) && !isset($this->_exchangeList[$exchangeName])){
            $return = array();
            foreach($d as $k=>$v){
                $return[]=array('data'=>$v,'p'=>$this->_p,'code'=>104,'routingkey'=>$routingkey,'exchange'=>$exchangeName,'msg'=>'exchange:'.$exchangeName .' not exist');
            }
            return $return;
        }
        ...
        foreach($d as $k=>$v){
            $data = $this->encodeData($v,$this->_config['format']);
            $msg = new AMQPMessage($data,$this->_p);
    ...
    public function waitConfirm($write_timeout=null){
        ...
        if($this->_confirm_select){
            ...
            if($r = $this->_channel->getNoAckMessages()){
                foreach($r as $k=>$msg){
                    $return[]=array('data'=>$msg->body,'p'=>$msg->get_properties(),'code'=>102,'vhost'=>$this->_vhost,'exchange'=>$msg->exchange,'route'=>$msg->routingKey,'msg'=>'timeout or server no response');
                }
            }
            if($beReGetChannel){
                $this->reGetChannel();
    ...
    * return bool
    */
    public function setDataByBatch($data,$routingkey=null,$exchangeName='amq.direct'){
        $this->_initQueueByRoutingKey($exchangeName,$routingkey);
    ...
    * return bool
    */
    public function setData($data,$routingkey=null,$exchangeName='amq.direct'){
        if(!$routingkey && $exchangeName == 'amq.direct'){
            throw new \Exception('请传入routingkey 或者非direct模式的交换机');
        }
        if($this->_batch_publish){
            return $this->setDataByBatch($data,$routingkey,$exchangeName);
        }
        $data = $this->encodeData($data,$this->_config['format']);
        $this->_initQueueByRoutingKey($exchangeName,$routingkey);
    ...
    */
    public function _callbackFunction($msg){
        ...
        if ($this->_logCallFunc){
            call_user_func($this->_logCallFunc,array('body'=>$data,'properties'=>$msg->get_properties(),'vhost'=>$this->_vhost,'queue'=>$this->_consumerQueueName),2);
        }
