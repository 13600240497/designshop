...
function openCustomLayoutDialog (target, prevId, isDroppedInParent) {
	layui.layer.open({
		...
		btn2: function (index) {
			...
			var url = '/activity/dl/layout-design/add-custom-layout'
			var data = {
				prev_id: prevId,
				page_id: $('#pageId').val(),
...
Design.dropInParent = function (type) {
	...
	if ($(this.get('gTarget')).closest('.design-view').length > 0) {
		if (type === 'layout') {
			url = '/activity/dl/layout-design/move-layout'
			data = {
				page_id: $('#pageId').val(),
			...
			url = '/activity/dl/ui-design/move-ui'
			data = {
				page_id: $('#pageId').val(),
		...
		if (type === 'layout') {
			url = '/activity/dl/layout-design/add-layout'
			data = {
				page_id: $('#pageId').val(),
			...
			url = '/activity/dl/ui-design/add-ui'
			data = {
				page_id: $('#pageId').val(),
				layout_id: $(this.get('pTarget')).closest(this.toClassName('lDrag')).attr('id'),
				component_key: $(this.get('gTarget')).data('key'),
...
$(function () {
	...
	$('.template-item-select-container').on('click', '.js_tmpConfirmSelect', function () {
		...
		if (item.length) {
			...
			layui.layer.confirm('切换所需模板后，参数会被替换。', {
				...
				$.post('/activity/dl/page-tpl/confirm-tpl', {
					tpl_id: id,
					page_id: $('#pageId').val(),
					lang: $('#pageLang').val()
				}, function (res) {
	...
	$('#sync_goodsList').click(function () {
		layui.layer.confirm('是否同步英文版商品sku？', {
			...
			$.ajax({
				url: '/activity/dl/design/copy-sku',
				type: 'POST',
	...
	$('#sync_config').click(function () {
		layui.layer.confirm('是否同步英文版数据配置？', {
			...
			$.get('/activity/dl/design/copy-page', {
				lang: $('#pageLang').val(),
				page_id: $('#pageId').val()
			}, function (res) {
				if (res.code == 0) {
	...
	$('#add_page_stylesheet').click(function () {
		$.get('/activity/dl/design/get-setting', {
			page_id: $('#pageId').val(),
			lang: $('#pageLang').val()
		}, function (res) {
			if (res.code == 0) {
				layui.layer.open({
					...
					// },
					btn2: function () {
						var url = '/activity/dl/design/setting'
						// var data = {
						// 	page_id: $('#pageId').val(),
						// 	lang: $('#pageLang').val(),
	...
	$('#generate_page_model').click(function () {
		layui.layer.open({
			...
			success: function () {
				...
				layui.upload.render({
					...
					field: 'files',
					url: '/activity/dl/page-tpl/upload-pic',
					accept: 'images',
			...
			btn2: function (index) {
				var url = '/activity/dl/page-tpl/add'
				var data = {
					pageId: $('#pageId').val(),
					lang: $('#pageLang').val(),
		...
		Design.postAjax('/activity/dl/page-tpl/get-snapshot', {
			site_code: siteCode,
			action: 'activity',
			id: pageLanguageId
		}, function (res) {
	...
	$('#generate_page').click(function () {
		layui.layer.open({
			...
			btn2: function (index) {
				...
				Design.postAjax('/activity/dl/design/release', {
					page_id: pageId,
	...
	$('body').on('click', '.redistribution', function (index) {
		$.post('/activity/dl/design/release', {
			page_id: $('#pageId').val(),
			lang: $('#pageLang').val()
		}, function (res) {
			layui.layer.msg(res.message)
	...
	$('#view_page').click(function () {
		$.get('/activity/dl/page/get-page-newest-urls', {
			id: $('#pageId').val()
		}, function (res) {
			Design.disableLoading()
			if (res.code == 0) {
	...
	function openComponentForm (componentId, isNewTemplate, isNewForm) {
		...
		$.get('/activity/dl/ui-design/get-form', {
			page_id: $('#pageId').val(),
			id: componentId,
			lang: $('#pageLang').val(),
			...
			if (res.code === 0) {
				...
				setTimeout(function () {
					...
					form.on('radio(templateId)', function (event) {
						var url = '/activity/dl/ui-design/save-form'
						var data = {
							page_id: $('#pageId').val(),
	...
	$('#design_view')
		...
		.on('click', '.js_openLayoutForm', function () {
			...
			Design.enableLoading()
			Design.componentFormShow()
			$.get('/activity/dl/layout-design/get-layout-form', {
				page_id: $('#pageId').val(),
				id: id
			}, function (res) {
				Design.disableLoading()
		...
		.on('click', '.js_removeLayout', function () {
			...
			layui.layer.confirm('确定删除该布局？', {
				...
				var url = '/activity/dl/layout-design/delete-layout'
		...
		.on('click', '.js_sortUpLayout', function () {
			...
			var url = '/activity/dl/layout-design/move-layout'
			var data = {
				page_id: $('#pageId').val(),
				id: target.attr('id'),
		...
		.on('click', '.js_removeComponent', function () {
			...
			layui.layer.confirm('确认删除该组件？', {
				...
				var url = '/activity/dl/ui-design/delete-ui'
		...
		.on('click', '.js_pasteInEmptyLayout', function () {
			...
			var url = '/activity/dl/ui-design/copy-ui'
			var data = {
				page_id: $('#pageId').val(),
		...
		.on('click', '.js_pasteInLayout', function () {
			...
			layui.layer.confirm('请输入复制份数', {
				...
				var url = '/activity/dl/layout-design/copy-layout'
				var data = {
					page_id: $('#pageId').val(),
					lang: $('#pageLang').val(),
		...
		.on('click', '.js_sortUpComponent', function () {
			...
			var url = '/activity/dl/ui-design/move-ui'
			var data = {
				page_id: $('#pageId').val(),
				id: target.data('id'),
	...
	$('.design-form')
		...
		.on('click', '#layout_form .js_submitDesignForm', function () {
			sessionStorage.removeItem('imgHeight')
			sessionStorage.removeItem('imgWidth')
			var url = '/activity/dl/layout-design/save-layout-form'
			var data = {
				page_id: $('#pageId').val(),
