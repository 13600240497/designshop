// for Zaful Design activity page
...
function openCustomLayoutDialog (target, prevId, isDroppedInParent) {
	layui.layer.open({
		...
		btn2: function (index) {
			...
			var url = '/activity/zf/layout-design/add-custom-layout'
			var data = {
				prev_id: prevId,
				page_id: $('#pageId').val(),
...
Design.enableLoading = function (bgColor) {
	$('#design_view_loading').removeClass('is-unActive').addClass('is-active')
}
...
Design.disableLoading = function () {
	$('#design_view_loading').removeClass('is-active').addClass('is-unActive')
	setTimeout(function () {
		$('#design_view_loading').removeClass('is-unActive')
	}, 600)
}
...
Design.dropInParent = function (type) {
	...
	if ($(this.get('gTarget')).closest('.design-view').length > 0) {
		if (type === 'layout') {
			url = '/activity/zf/layout-design/move-layout'
			data = {
				page_id: $('#pageId').val(),
			...
			url = '/activity/zf/ui-design/move-ui'
			data = {
				page_id: $('#pageId').val(),
		...
		if (type === 'layout') {
			url = '/activity/zf/layout-design/add-layout'
			data = {
				page_id: $('#pageId').val(),
			...
			url = '/activity/zf/ui-design/add-ui'
			data = {
				page_id: $('#pageId').val(),
				layout_id: $(this.get('pTarget')).closest(this.toClassName('lDrag')).attr('id'),
				component_key: $(this.get('gTarget')).data('key'),
...
$(function () {
	...
	$('.template-item-select-container').on('click', '.js_tmpConfirmSelect', function () {
		...
		if (item.length) {
			...
			layui.layer.confirm('切换所需模板后，参数会被替换。', {
				...
				$.post('/activity/zf/page-tpl/confirm-tpl', {
					tpl_id: id,
					page_id: $('#pageId').val(),
					lang: $('#pageLang').val()
				}, function (res) {
	...
	$('#sync_goodsList').click(function () {
		layui.layer.confirm('是否同步英文版商品sku？', {
			...
			$.ajax({
				url: '/activity/zf/design/copy-sku',
				type: 'POST',
	...
	$('#sync_config').click(function () {
		layui.layer.confirm('是否同步英文版数据配置？', {
			...
			$.get('/activity/zf/design/copy-page', {
				lang: $('#pageLang').val(),
				page_id: $('#pageId').val()
			}, function (res) {
				if (res.code == 0) {
	...
	$('#add_page_stylesheet').click(function () {
		$.get('/activity/zf/design/get-setting', {
			page_id: $('#pageId').val(),
			lang: $('#pageLang').val()
		}, function (res) {
			if (res.code == 0) {
				layui.layer.open({
						...
						if (list != null) {
							list.forEach(function (item, index) {
								...
								if (item.style_type == 1) {
									...
									// contentHtml+='<input type="radio" name="page_background_position_'+(index+1)+'"  value="buttom left" title="下左">'
					...
					// },
					btn2: function () {
						var url = '/activity/zf/design/setting'
						// var data = {
						// 	page_id: $('#pageId').val(),
						// 	lang: $('#pageLang').val(),
		...
		layui.layer.open({
			...
			success: function () {
				...
				layui.upload.render({
					...
					field: 'files',
					url: '/activity/zf/page-tpl/upload-pic',
					accept: 'images',
	...
	//三端绑定
	var multiple_plat_sync = document.getElementById('multiple_plat_sync')
	if(multiple_plat_sync){
		document.getElementById('multiple_plat_sync').addEventListener('click',function(){
			vm.$children[0].createdMirror()
		})
	}
	 ...
	 * 1. 需要判断是否原生专题链接，如果是的话处理一下，因为使用前端路由
	 */
	$('#view_convert_page').change(function () {
		 ...
		 * @param {string} url
		 */
		var is_native = function (url) { return url.includes('native'); };
		var url = $(this).val();
		if (is_native(url) == true) {
			url = url.replace('activity/zf/native-design/native', '#/design/native');
		}
		location.href = url;
	});
	...
	$('body').on('click', '.redistribution', function (index) {
		$.post('/activity/zf/design/release', {
			page_id: $('#pageId').val(),
			lang: $('#pageLang').val()
		}, function (res) {
			layui.layer.msg(res.message)
	...
	//多端绑定数据发布
	$('#generate_sync_platform_page').click(function(){
		var pageId = $('#pageId').val()
		Design.postAjax('/activity/zf/design/full-batch-release', {
			page_id: pageId,
		}, function (res) {
			layui.layer.msg(res.message)
			Design.disableLoading()
	...
	function openComponentForm (componentId, isNewTemplate, isNewForm) {
		...
		$.ajax({
			url: '/activity/zf/ui-design/get-form',
			method: 'get',
			data: {
				page_id: $('#pageId').val(),
				id: componentId,
				...
				if (res.code === 0) {
					...
					setTimeout(function () {
						...
						form.on('radio(templateId)', function (event) {
							var url = '/activity/zf/ui-design/save-form'
							var data = {
								page_id: $('#pageId').val(),
						...
						if(res.data.bind_relation){
							var $component_form = $('#component_form')
							$component_form.find('input[name=tpl_id]').prop('disabled',true)
							layui.form.render('radio')
							$component_form.find('.component-form-quote').after('<div style="padding: 5px 15px 0 10px;"><span class="form-sync-tips"></span><span style="color: #999999;\n' +
								'margin-left: 5px;">该组件已进行绑定，此处修改商品数据无效，请去三端绑定处操作</span></div>')
						}

						/*					//选品系统ips筛选回填
						 var localData_data = res.localData.data
						 if(localData_data.ipsMethods && (localData_data.ipsMethods === 3 || localData_data.ipsMethods === 4)){
						 $('input[name=ipsMethods]').prop('disabled',true);
						 layui.form.render('radio');
						 }

						 if(localData_data.goodsSKU instanceof Array){
						 localData_data.goodsSKU.forEach(function(item,index){
						 if( (item.ipsMethods == '3' || item.ipsMethods == '4')){
						 $('#component_form').find('.goods-tab-item:eq('+ (index+1) +')').find('.ipsItemRadio').prop('disabled',true)
						 }
						 })
						 layui.form.render('radio');
						 }*/
	...
	$('#design_view')
		...
		.on('click', '.js_openLayoutForm', function () {
			...
			Design.enableLoading()
			Design.componentFormShow()
			$.get('/activity/zf/layout-design/get-layout-form', {
				page_id: $('#pageId').val(),
				id: id
			}, function (res) {
				Design.disableLoading()
		...
		.on('click', '.js_removeLayout', function () {
			...
			layui.layer.confirm('确定删除该布局？', {
				...
				var url = '/activity/zf/layout-design/delete-layout'
		...
		.on('click', '.js_sortUpLayout', function () {
			...
			var url = '/activity/zf/layout-design/move-layout'
			var data = {
				page_id: $('#pageId').val(),
				id: target.attr('id'),
		...
		.on('click', '.js_removeComponent', function () {
			...
			layui.layer.confirm('确认删除该组件？', {
				...
				var url = '/activity/zf/ui-design/delete-ui'
		...
		.on('click', '.js_pasteInEmptyLayout', function () {
			...
			var url = '/activity/zf/ui-design/copy-ui'
			var data = {
				page_id: $('#pageId').val(),
		...
		.on('click', '.js_pasteInLayout', function () {
			...
			layui.layer.confirm('请输入复制份数', {
				...
				var url = '/activity/zf/layout-design/copy-layout'
				var data = {
					page_id: $('#pageId').val(),
					lang: $('#pageLang').val(),
		...
		.on('click', '.js_sortUpComponent', function () {
			...
			var url = '/activity/zf/ui-design/move-ui'
			var data = {
				page_id: $('#pageId').val(),
				id: target.data('id'),
		...
		.on('click', '.js_saveComponentTemp', function () {
			...
			layui.layer.open({
				...
				btn2: function (index) {
					var url = '/activity/zf/page-ui-tpl/add'
					var data = {
						page_id: $('#pageId').val(),
						ui_id: ui_id,
			...
			*/
			return false
			Design.postAjax('/activity/zf/page-ui-tpl/get-snapshot', {
				ui_id: ui_id,
				page_id: $('#pageId').val()
			}, function (res) {
				Design.disableLoading()
	...
	$('.design-form')
		...
		.on('click', '#layout_form .js_submitDesignForm', function () {
			sessionStorage.removeItem('imgHeight')
			sessionStorage.removeItem('imgWidth')
			var url = '/activity/zf/layout-design/save-layout-form'
			var data = {
				page_id: $('#pageId').val(),
	...
	function renderColorPicker () {
		...
		$('.js-color-tracker').each(function() {
			var dom = $(this)
			var input = dom.find('input')
			var icon = dom.find('.js-color-tracker-icon')
			var colorPicker = input.colorPicker({
				klass: window.ColorPicker,
				input: dom,
				initStyle: 'z-index: 1111111',
				animationSpeed: 0,
				...
				renderCallback: function(colors, mode) {
					var value = ''
					if (colors.alpha < 1) {
						value = 'rgba(' + colors.RND.rgb.r +', ' + colors.RND.rgb.g +', ' + colors.RND.rgb.b +', ' + colors.alpha + ')'
					} else {
						value = '#' + colors.HEX
					}
					input.val(value)
