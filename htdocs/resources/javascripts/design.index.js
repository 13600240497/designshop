...
function openCustomLayoutDialog (target, prevId, isDroppedInParent) {
	layui.layer.open({
		...
		btn2: function (index) {
			...
			var url = '/home/layout-design/add-custom-layout'
			var data = {
				prev_id: prevId,
				page_id: $('#pageId').val(),
...
Design.dropInParent = function (type) {
	...
	if ($(this.get('gTarget')).closest('.design-view').length > 0) {
		if (type === 'layout') {
			url = '/home/layout-design/move-layout'
			data = {
				page_id: $('#pageId').val(),
		...
		if (type === 'layout') {
			url = '/home/layout-design/add-layout'
			data = {
				page_id: $('#pageId').val(),
...
$(function () {
	...
	$('#view_page').click(function () {
		...
		if (Number(status) == 8) {
			url = '/home/page/get-homeb-newest-urls'
		} else {
			url = '/home/page/get-page-newest-urls'
		}
		...
		layui.layer.open({
			...
			success: function () {
				...
				if (status && status != 1 && status != 3) {
					$('#ab_test_lang_wraper').show()
				} else {
					$('#ab_test_lang_wraper').hide()
				}
			},
			...
			btn2: function(index, layDom){
				...
				var url = indexType === 1 ? '/home/design/online' : '/home/design/online-b'
				...
				var langArr = []
				$('#ab_test_lang_wraper').find('.layui-form-checked').each(function() {
					var $this = $(this)
	...
	form.on('radio(templateId)', function (event) {
		var url = '/home/ui-design/save-form'
		var data = {
			page_id: $('#pageId').val(),
	...
	$('#design_view')
		...
		.on('click', '.js_openLayoutForm', function () {
			...
			Design.enableLoading()
			Design.componentFormShow()
			$.get('/home/layout-design/get-layout-form', {
				page_id: $('#pageId').val(),
				id: id
			}, function (res) {
				Design.disableLoading()
		...
		.on('click', '.js_removeLayout', function () {
			...
			layui.layer.confirm('确定删除该布局？', {
				...
				var url = '/home/layout-design/delete-layout'
		...
		.on('click', '.js_sortUpLayout', function () {
			...
			var url = '/home/layout-design/move-layout'
			var data = {
				page_id: $('#pageId').val(),
				id: target.attr('id'),
		...
		.on('click', '.js_removeComponent', function () {
			...
			layui.layer.confirm('确认删除该组件？', {
				...
				var url = '/home/ui-design/delete-ui'
		...
		.on('click', '.js_pasteInEmptyLayout', function () {
			...
			var url = '/home/ui-design/copy-ui'
			var data = {
				page_id: $('#pageId').val(),
		...
		.on('click', '.js_pasteInLayout', function () {
			...
			layui.layer.confirm('请输入复制份数', {
				...
				var url = '/home/layout-design/copy-layout'
				var data = {
					page_id: $('#pageId').val(),
					lang: $('#pageLang').val(),
		...
		.on('click', '.js_sortUpComponent', function () {
			...
			var url = '/home/ui-design/move-ui'
			var data = {
				page_id: $('#pageId').val(),
				id: target.data('id'),
		...
		.on('click', '.js_saveComponentTemp', function () {
			...
			*/
			return false;
			Design.postAjax('/home/page-ui-tpl/get-snapshot', {
				ui_id: ui_id,
				page_id: $('#pageId').val()
			}, function (res) {
				Design.disableLoading()
	...
	$('.design-form')
		...
		.on('click', '#layout_form .js_submitDesignForm', function () {
			sessionStorage.removeItem('imgHeight')
			sessionStorage.removeItem('imgWidth')
			var url = '/home/layout-design/save-layout-form'
			var data = {
				page_id: $('#pageId').val(),
