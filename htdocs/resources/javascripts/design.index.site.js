...
function getPublicMenus () {
	$.ajax({
		url: '/base/menu/public-menus',
		type: 'GET',
		data: { page_id: $('#pageId').val() },
...
function openCustomLayoutDialog (target, prevId, isDroppedInParent) {
	layui.layer.open({
		...
		btn2: function (index) {
			...
			var url = '/home/zf/layout-design/add-custom-layout'
			var data = {
				prev_id: prevId,
				page_id: $('#pageId').val(),
...
Design.componentFormShowAdvancedItem = function () {
	$('.component-form-templatelist-item,.component-form-configure-item').hide()
	$('.component-form-advanced-item').show().addClass('show')
}
Design.componentFormHideAdvancedItem = function () {
	$('.component-form-advanced-item').hide().removeClass('show')
	$('.component-form-templatelist-item,.component-form-configure-item').show()
}

Design.componentFormShowTempListItem = function () {
	$('.component-form-template-item,.component-form-configure-item').hide()
	// $('.component-form-templatelist-item').show().addClass('show')
}
Design.componentFormHideTempListItem = function () {
	// $('.component-form-templatelist-item').hide().removeClass('show')
	$('.component-form-template-item,.component-form-configure-item').show()
}
...
Design.dropInParent = function (type) {
	...
	if ($(this.get('gTarget')).closest('.design-view').length > 0) {
		if (type === 'layout') {
			url = '/home/zf/layout-design/move-layout'
			data = {
				page_id: $('#pageId').val(),
		...
		if (type === 'layout') {
			url = '/home/zf/layout-design/add-layout'
			data = {
				page_id: $('#pageId').val(),
...
$(function () {
	...
	// 模板列表展开收起
	$('body').on('click', '#js_showMoreTemplate', function () {
		var _this = $(this)
		var $list = $('.component-form-template-container .component-tmp-list:gt(1)')
	...
	$('.template-item-select-container').on('click', '.js_tmpConfirmSelect', function () {
		...
		if (item.length) {
			...
			layui.layer.confirm('切换所需模板后，参数会被替换。', {
				...
				$.post('/home/zf/page-tpl/confirm-tpl', {
					tpl_id: id,
					page_id: $('#pageId').val(),
					lang: $('#pageLang').val()
				}, function (res) {
	...
	$('#add_page_stylesheet').click(function () {
		$.get('/home/zf/design/get-setting', {
			page_id: $('#pageId').val(),
			lang: $('#pageLang').val()
		}, function (res) {
			if (res.code == 0) {
				layui.layer.open({
					...
					// },
					btn2: function () {
						var url = '/home/zf/design/setting'
						var data = {
							page_id: $('#pageId').val(),
							lang: $('#pageLang').val(),
						}
	 ...
	 */
	$('#view_convert_page').click(function () {
...
/*                if (status != 1 && status != 3) {
	...
	// 发布并设为首页
	$('#generate_and_set').click(function () {
		layui.layer.open({
			type: 1,
			title: '请选择设为首页类型',
			content: $('#ab_test').html(),
			...
			success: function () {
				...
				if (status && status != 1 && status != 3) {
					$('#ab_test_pipeline_wraper').show()
				} else {
					$('#ab_test_pipeline_wraper').hide()
				}
				var all_home_permissions =  typeof GESHOP_HAS_ALL_HOME_PERMISSIONS !== 'undefined' ? GESHOP_HAS_ALL_HOME_PERMISSIONS : null
				// 如果当前是B首页并且没有权限时无法发布A首页
				if (typeof GESHOP_IS_HOME_B !== 'undefined' && GESHOP_IS_HOME_B == '1') {
					if (all_home_permissions == 0) {
						$('#ab_test_temp').find('input[value=1]').attr('disabled', true)
						$('#ab_test_temp').find('input[value=2]').prop('checked',true)
					} else {
						$('#ab_test_temp').find('input[value=1]').attr('disabled', false)
					}
				}
			},
			yes: function(index, layDom){
		...
		// layui.layer.confirm('确认生成该页面并设为首页？', {
		// 	btn: ['取消', '确定'],
		// 	area: '420px',
		// 	skin: 'element-ui-dialog-class',
		// 	icon: 3,
	...
	layui.use('form', function () {
        ...
        // 版本锁定解锁
        form.on('switch(page_lock)', function (data) {
            if (data.elem.checked) {
                $('#page_lock').prop('disabled',false);
            } else {
                $('#page_lock').prop('disabled',true);
                $.get('/base/zf/home-log/remove-rollback-lock', {
                    site_code: $('#siteCode').val(),
                    page_id: $('#pageId').val()
                }, function (res) {
                    if (res.code == 0) {
                        layui.layer.msg(res.message);
                        setTimeout(function(){
                            $('#page_lock').parent().addClass('layui-hide');
                        },200)
                    } else {
                        layui.layer.msg(res.message)
                        $('#page_lock').prop({'checked':true,'disabled':false})
	...
	form.on('radio(templateId)', function (event) {
		var url = '/home/zf/ui-design/save-form'
		var data = {
			page_id: $('#pageId').val(),
		...
		var successCallBack = function (res) {
			Design.disableLoading()
			if (res.code === 0) {
				// console.log('event.elem.value', event.elem.value);
	...
	// 模板切换
	$('body').on('click', '.js_changeComponentTemplate', function () {
		var id = $(this).data('id')
		$('input[name=tpl_id]').val(id)

		if (typeof id != 'undefined') {
			$(this).addClass('component-tmp-current').siblings().removeClass('component-tmp-current')
		}
		...
		// 是否需要触发重新组装数据
		// if ($('.js_submitDesignForm').data('tag') == '1') {
		// 	sessionStorage.setItem('currentTemplateId', id) // 当前模板id
		// 	sessionStorage.setItem('isTriggerTemplate', '1') // 切换模板标识
		// 	$('.js_submitDesignForm').prev().trigger('click')
		// } else {
		// 	Design.postAjax(url, data, successCallBack, errorCallBack)
		// }
		Design.postAjax(url, data, successCallBack, errorCallBack)
	...
	function openComponentForm (componentId, isNewTemplate, isNewForm) {
		...
		$.get('/home/zf/ui-design/get-form', {
			page_id: $('#pageId').val(),
			id: componentId,
			lang: $('#pageLang').val(),
			tpl_id: sessionStorage.getItem('currentTemplateId')
				...
				setTimeout(function () {
					...
					$('body')
						.on('click', '#js_moreConfig', function () {
							Design.componentFormShowAdvancedItem()
						})
						.on('click', '#js_advanced_baseConfig', function () {
							Design.componentFormHideAdvancedItem()
						})
						.on('click', '#js_showMoreTemplate', function () {

						})
					// .on('click', '#js_templateList', function () {
					// 	Design.componentFormShowTempListItem()
					// })
					// .on('click', '#js_templatelist_baseConfig', function () {
					// 	Design.componentFormHideTempListItem()
					// })
					renderColorPicker()
	...
	$('#design_view')
		...
		.on('mouseenter', Design.toClassName('cDrag'), function () {
			...
			if (target.find('.design-operation-panel').length === 0) {
				...
				if (isUnique) {
					target.find('.js_copyComponent').remove()
				}
			...
			if (layoutDragTarget.find('> .design-operation-panel').length === 0) {
				...
				if (isUnique) {
					layoutDragTarget.find('.js_copyLayout').remove()
				}
		...
		.on('click', '.js_openLayoutForm', function () {
			...
			Design.enableLoading()
			Design.componentFormShow()
			$.get('/home/zf/layout-design/get-layout-form', {
				page_id: $('#pageId').val(),
				id: id
			}, function (res) {
				Design.disableLoading()
		...
		.on('click', '.js_removeLayout', function () {
			...
			layui.layer.confirm('确定删除该布局？', {
				...
				var url = '/home/zf/layout-design/delete-layout'
		...
		.on('click', '.js_sortUpLayout', function () {
			...
			var url = '/home/zf/layout-design/move-layout'
			var data = {
				page_id: $('#pageId').val(),
				id: target.attr('id'),
		...
		.on('click', '.js_removeComponent', function () {
			...
			layui.layer.confirm('确认删除该组件？', {
				...
				var url = '/home/zf/ui-design/delete-ui'
		...
		.on('click', '.js_pasteInEmptyLayout', function () {
			...
			var url = '/home/zf/ui-design/copy-ui'
			var data = {
				page_id: $('#pageId').val(),
		...
		.on('click', '.js_pasteInLayout', function () {
			...
			layui.layer.confirm('请输入复制份数', {
				...
				var url = '/home/zf/layout-design/copy-layout'
				var data = {
					page_id: $('#pageId').val(),
					lang: $('#pageLang').val(),
		...
		.on('click', '.js_sortUpComponent', function () {
			...
			var url = '/home/zf/ui-design/move-ui'
			var data = {
				page_id: $('#pageId').val(),
				id: target.data('id'),
		...
		.on('click', '.js_saveComponentTemp', function () {
			...
			layui.layer.open({
				...
				btn2: function (index) {
					var url = '/home/zf/page-ui-tpl/add'
					var data = {
						page_id: $('#pageId').val(),
						ui_id: ui_id,
			...
			*/
			return false;
			Design.postAjax('/home/zf/page-ui-tpl/get-snapshot', {
				ui_id: ui_id,
				page_id: $('#pageId').val()
			}, function (res) {
				Design.disableLoading()
	...
	$('.design-form')
		...
		.on('click', '#layout_form .js_submitDesignForm', function () {
			sessionStorage.removeItem('imgHeight')
			sessionStorage.removeItem('imgWidth')
			var url = '/home/zf/layout-design/save-layout-form'
			var data = {
				page_id: $('#pageId').val(),
	...
	$('body')
		.on('click', '.js_useResource', function () {
			...
			if (currentTabContentType == 'tabBgImg') {
				...
				if (isSelect && !isRight && !target.data('skipValid')) {
