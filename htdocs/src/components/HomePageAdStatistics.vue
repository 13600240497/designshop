<template>
	<site-layout>
		<el-row :span="24" class="gs-activity-ad-tit">
			<span class="gs-activity-ad-title">首页广告位统计</span>
		</el-row>
      ...
      <el-col class="gs-activity-ad-port">
        ...
        <!-- <span class="gs-activity-ad-port-name" style="width:46px;">请选择</span> -->
				<el-select v-model="search.homePage" filterable clearable placeholder="请输入或选择" @change="handleChangeHomePage" @visible-change="handleChangeHomePageList" class="gs-activity-ad-port-select">
          <el-option
            v-for="item in handleHomePageList(homePageList)"
            :key="item.id"
            :label="item.title"
            :value="item.id">
          </el-option>
      ...
      <el-col class="gs-activity-ad-port activity-select-position">
				<el-select v-model="search.position" @change="handleChangePosition" placeholder="查看">
          <el-option label="广告位查看" value="1"></el-option>
          <el-option label="坑位查看" value="2"></el-option>
        </el-select>
			</el-col>
      <el-col class="gs-activity-ad-port activity-select-component" v-if="search.position == '2'">
      ...
      <!-- <el-col class="gs-activity-ad-port" style="width: 124px !important;">
				<el-select v-model="search.componentId" placeholder="">
          <el-option 
      ...
      <el-col class="gs-activity-ad-time" v-if="this.search.position == 1">
          ...
          <el-radio-button v-for="item in periodList" :key="item.label" :label="item.label">{{ item.name }}</el-radio-button>
        </el-radio-group>
			</el-col>
      ...
      <el-col class="gs-activity-ad-export">
		...
		<el-row class="gs-activity-ad-time-container" v-if="this.search.position == 2">
    ...
    <div class="gs-activity-ad-table">
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="empty" v-if="search.position == '1'" label="点击率" width="108">
        </el-table-column>
        <el-table-column prop="pit_cl_rate" v-if="search.position == '2'" label="点击率" width="108">
        </el-table-column>
...
<script>
  ...
  import { getHomePageList, getReportHomeAdList, getHomePageComponentList } from "../plugin/api"
  ...
  export default {
    ...
    watch: {
      'search.homePageID': {
        handler (val, oldVal) {
          if (val) {
            this.search.homePage = this.search.homePageID
            if (this.search.homePageID) {
              // 获取报表数据（page_id必填，默认选择default字段值作为page_id参数）
              this.getHomePageDataReportFn()
            }
          }
        },
        deep: true
    ...
    methods: {
       ...
       * 列表数据处理
       */
      handleHomePageList (homePageList) {
        let pages = []
      ...
      async getHomePageList() {
        ...
        let res = await getHomePageList(param)
        if (res.code == 0) {
          this.homePageList = res.data.list
          this.search.homePageID = res.data.default ? res.data.default.id : ''
       ...
       */
      async getHomePageDataReportFn() {
        let param = {
          platform: this.search.platformValue,
          view_type: this.search.position,
          page_id: this.search.homePage || 0,
        ...
        let res = await getReportHomeAdList(param)
      ...
      exportExcel() {
        ...
        let href = `${location.origin}/base/report/home-page-detail-data-list?platform=${param.platform}&view_type=${param.view_type}&page_id=${param.page_id}&pageSize=${param.pageSize}&pageNo=${param.pageNo}&excel=1&start_time=${param.start_time}&end_time=${param.end_time}`
       ...
       * 获取首页列表
       */
      handleChangeHomePageList (msg) {
        if (msg) {
          // 清空搜索框
          this.search.homePage = ''
          this.getHomePageList()
        }
      },
      ...
      handleChangeHomePage () {
        this.getHomePageComponentListFn()
      },
       ...
       */
      async getHomePageComponentListFn() {
        let param = {
          page_id: this.search.homePage
        }
        let res = await getHomePageComponentList(param)
