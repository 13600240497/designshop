	...
	<site-layout @publicReady="publicReady" :footLink="1">
		...
		<el-row class="geshop-Activity-btn">
			<el-col :span="24" class="gb-search-form" style="min-width:320px">
				...
				<el-button @click="refreshHeadFoot" class="geshop-gb-btn-refresh" style="left:180px;">
									...
									<el-dropdown split-button style="margin-left:192px;">
										<el-dropdown-menu slot="dropdown">
											...
											<el-dropdown-item type="danger" size="small"
																				@click.native="removePage(list, list.is_lock, list.activity_create_user)">删除
											</el-dropdown-item>
										</el-dropdown-menu>
									</el-dropdown>
								</div>
			...
			<el-form :model="activityForm" :rules="activityRules" ref="activityForm" v-loading="obsLoading">
				...
				<el-form-item label="渠道" prop="channelLang" class="geshop-new-activities-place" v-if="activityForm.status==1">
					...
					<el-tabs v-model="activityForm.channelCurrent">
						<el-tab-pane v-for="(item,key) in dynamicPipeLineObj_activity" :label="item.name" :name="item.key"
												 :key="item.key">
			...
			<el-form :model="activityChannelForm" ref="activityChannelForm">
				...
				<el-form-item label="渠道" prop="channelLang" class="geshop-gb-activities-place">
					...
					<el-tabs v-model="activityChannelForm.channelCurrent" v-if="dialogChannelAddVisible">
						<el-tab-pane v-for="item in dynamicPipeLineObj_activity" :label="item.name" :name="item.key" :key="item.key">
							<el-form-item label="语言" prop="lang">
							...
							<el-tabs type="card" @tab-click="handleTabClick($event,'lang')" v-model="currentLanguage">
								...
								<el-form-item label="分享图片" prop="share_img" class="child-page-keywords child-page-share_img">
									...
									<el-upload action="/activity/page-tpl/upload-pic" name="files" accept="image/jpg,image/jpeg,image/png"
										...
										<div slot="tip" class="el-upload__tip">注：建议上传600x314的jpg\png图片</div>
...
<script>
	...
	import { getCookie, extendDeep,mapPipeLine,getPlatformPageLang } from '../plugin/mUtils';
		...
		data () {
			return {
				...
				activityChannelForm: {
					...
					channelLang_old: [],
					channelCurrent: '',
					pipelineList: []	//活动row.group_info.pipelineList
				},
			...
			'activityForm.place':{
				handler(newValue){
					if(newValue && newValue.length>0 && typeof newValue[0] === 'string'){
						...
						let dynamicPipeLineObj_activity = mapPipeLine(allSupportChannelLang_res.data,newValue);
						this.dynamicPipeLineObj_activity = dynamicPipeLineObj_activity;
						//更新活动语言选中
						this.activityForm.channelLang = this.channelLangInit(dynamicPipeLineObj_activity, 'createActivity');
					}else{
			...
			'activityChannelForm.editPlace':{
				handler(newValue){
					if(newValue && typeof newValue[0] === 'string'){
						...
						//更新活动语言选中
						let channelLangAll_from_blank = this.channelLangInit(dynamicPipeLineObj_activity, 'createActivity',true),
							channelLang_old = JSON.parse(JSON.stringify(channelLangAll_from_blank)),
							editSupportChannelLang = this.activityChannelForm.pipelineList;

						this.addActivityChannel_checkbox(channelLangAll_from_blank,channelLang_old,editSupportChannelLang)
					}
...
/*				var data = this.pageForm.url_name.split('');
			...
			editPage (row, is_lock, activity_create_user, langList, placeList) {
				...
				if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
					...
					// let	channelGroup = this.channelGroup;
					let currentPlatLang = row.group_languages[this.activityTabName];
					if(currentPlatLang){
						let editPipeLine = getPlatformPageLang(row.group_languages[this.activityTabName])
						this.transformChannel(editPipeLine);
					}
				...
				if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
					...
					this.confirm('确认删除该页面?', async (vm) => {
						let params = {
							...
							res = await GB_deletePage(params);
		...
		*/
			convertAPP (id, is_lock, activity_create_user, group_info, group_languages, list) {
				if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
					...
					this.convertForm.source_channelLang = this.channelLangInit(group_languages.wap);
					this.convertForm.ios_target_channelLang = this.channelLangInit(group_languages.ios);
					this.convertForm.android_target_channelLang = this.channelLangInit(group_languages.android);
			...
			convertNative (convertType, id, is_lock, activity_create_user, group_info, group_languages, list) {
				if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
					...
					let $form = this.convertForm_ios_android;
					$form.source_channelLang = this.channelLangInit(convertType== 'ios2android' ? group_languages.ios : group_languages.android);
					$form.target_channelLang = this.channelLangInit(convertType== 'ios2android' ? group_languages.android : group_languages.ios);
			...
			convertM (id, is_lock, activity_create_user, group_info, group_languages, rowList) {
				if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
					...
					this.convertForm_M.source_channelLang = this.channelLangInit(group_languages.pc);
					this.convertForm_M.target_channelLang = this.channelLangInit(group_languages.wap);
			...
			handleSelectAllLang (value, currentChannel) {
				if (value) {
					let dynamicPipeLine = this.dynamicPipeLineObj_activity[currentChannel];
					let currentLangAll = dynamicPipeLine ? dynamicPipeLine.language : {};
			...
			/**
			 * 新增活动渠道checkbox 选中
			 * @param channelLangAll_from_blank 空白渠道语言列表
			 * @param channelLang_old 空白渠道语言列表备份
			 * @param editSupportChannelLang 当前row.group_info.pipelineList
			 */
			addActivityChannel_checkbox(channelLangAll_from_blank,channelLang_old,editSupportChannelLang){
				editSupportChannelLang.forEach((item, index) => {
					item.langList.forEach((childItem) => {
			...
			addActivityChannel (row) {
				...
				//填充已选渠道
				this.activityChannelForm.pipelineList = row.group_info.pipelineList;
				row.group_info.pipelineList.forEach((item, index) => {
					item.langList.forEach((childItem) => {
						if(channelLangAll_from_blank[item.key]){
							channelLangAll_from_blank[item.key]['value'].push(childItem.key);
