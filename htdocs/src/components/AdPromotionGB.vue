<template>
	<site-layout @publicReady="publicReady">
		...
		<el-row class="geshop-Activity-btn">
			<el-col>
				<el-button class="geshop-Activity-btn-add" @click="createActivity">
					...
					<span class="geshop-icon-add-text">新增广告落地页</span>
				</el-button>
			</el-col>
			<!-- <el-col>
				<el-button class="geshop-Activity-btn-refresh" @click="refreshHeadFoot" style="left: 200px;">
					<span class="icon-geshop-reset"></span>
					<span class="geshop-icon-refresh-text">一键刷新头尾部</span>
				</el-button>
			</el-col> -->
			<el-col>
				<el-input class="geshop-index-btn-pageid" v-model="subpageId" placeholder="请输入子页面ID"></el-input>
				<el-input class="geshop-index-btn-search" v-model="searchWord" placeholder="请输入广告落地页名称"></el-input>
				<el-button class="geshop-index-btn-dosearch" @click="doSearch" type="primary">搜索</el-button>
				 ...
				 :expand-row-keys="expandRowKeys" @row-click="handleExpandChange">
								...
								<div>
									...
									<div class="child-pages-hover">
										...
										<el-tooltip content="转M端" placement="bottom" effect="light">
											<el-button class="icon-geshop-mobile" @click="convertM(list.id, list.is_lock, list.activity_create_user, list.group_info, list.group_languages)"
					...
					<el-table-column label="操作" width="450" class="geshop-activity-more" class-name="ge-activity-list-operating">
						<template slot-scope="scope" style="line-height:80px;">
							...
							<el-switch v-model="scope.row.lock_status" @change="gbAdLockingActivity(scope.row.id, scope.row.is_lock, scope.row.create_user, $event)"></el-switch>
						</template>
					</el-table-column>
				</el-table>
			</el-col>
		...
		<el-dialog title="列表页点击预览" :visible.sync="dialogChannelVisible">
			<el-row>
				<el-button v-for="item in channelUrlList" style="width: 148px; height: 42px; font-size: 14px; margin: 0 0 10px 5px; text-overflow: ellipsis; overflow: hidden;" type="primary" :key="item.name" @click="redirect(item.link)">{{
					item.name }}
				</el-button>
				<!-- <p>{{ tips }}
		...
		<el-dialog :title="activityForm.dialogTitle" :visible.sync="dialogActivityVisible" class="geshop-new-activities">
			<el-form :model="activityForm" :rules="activityRules" ref="activityForm">
				...
				<p style="margin:0;color:#b7b7b7;">(注：勾选内容表示此活动信息新增到的渠道）</p>
				<!-- 编辑状态语言 -->
				<el-form-item label="渠道" prop="lang" class="geshop-new-activities-lang" v-if="activityForm.status==2">
				...
				<el-form-item label="简介" prop="description" class="geshop-new-activities-introduction" v-on:keyup.native="handleIntroductionKeyup">
					<el-input type="textarea" v-model="activityForm.description" :rows="4" placeholder="请简单描述一下这个广告落地页......"></el-input>
					<span class="count-tip-box">{{ actIntroductionCount }}/200</span>
				</el-form-item>
				<p style="margin:5px 0 5px 0;color:#b7b7b7;">注：勾选应用端口及渠道提交后不可再修改了哦！</p>
		...
		<el-dialog :title="pageForm.dialogTitle" :visible.sync="dialogPageVisible" class="geshop-new-child-page" @close="subDialogClose">
			<el-form :model="publicPageForm" :rules="publicPageRules" ref="publicPageForm">
				...
				<el-form-item label="下线时间" prop="end_time" class="gs-col-all" style="margin:0px 0px 10px">
					<el-date-picker v-model="end_time" type="datetime" :disabled="this.currentLanguage != this.firstLanguage"
			...
			<el-form :model="pageForm" :rules="pageRules" ref="pageForm" class="geshop-new-child-page-title">
				<el-form-item label="名称" prop="title" class="gs-col-all">
					<el-input v-model="pageForm.title" @change="updatePageTitle" v-on:keyup.native="handleTitleKeyup"></el-input>
				</el-form-item>
				<el-form-item label="页面url" prop="url_name" class="gs-col-all child-page-url">
				...
				<!-- <el-form-item label="主商品sku" prop="goods_sn" class="child-page-link goods_sn-input">
					<el-row :gutter="20">
						<el-col :span="15" style="width:200px;margin-right:20px;">
							<el-input v-model="pageForm.goods_sn" @change="updatePagegoods_sn" @blur="handleGoodsSkuLink" style="width:200px;"></el-input>
						</el-col>
						<span v-if="pageForm.page_url != ''" style="display: inline-block;max-width: 400px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;">推广链接：{{
							pageForm.page_url }}</span>
						<el-button type="primary" v-if="pageForm.page_url != ''" style="display:inline-block;margin-left:15px;vertical-align:top;"
						 @click="handleCopySkuLink"><span v-if="pageForm.copyStatus == 1">复制</span><span v-else>已复制</span></el-button>
				...
				<!-- <el-form-item label="分享标题" prop="shareTitle" class="gs-col-all">
					<el-input v-model="pageForm.shareTitle" @change="updatePageShareField($event,'shareTitle')"></el-input>
				</el-form-item>
				<el-form-item label="分享描述" prop="shareDesc" class="gs-col-all">
					<el-input v-model="pageForm.shareDesc" @change="updatePageShareField($event,'shareDesc')"></el-input>
				</el-form-item>
				<el-form-item label="分享图片" prop="shareImage" class="gs-col-all">
					<el-input v-model="pageForm.shareImage" @change="updatePageShareField($event,'shareImage')"></el-input>
				</el-form-item> -->
		...
		<el-dialog title="活动访问地址" id="dialogLinkBox" :visible.sync="dialogLinksVisible">
			<el-row>
				<el-button v-for="item in pageLinks" style="width: 148px; height: 42px; font-size: 14px; margin: 0 0 10px 5px; text-overflow: ellipsis; overflow: hidden;" type="primary" :key="item.lang" @click="redirect(item.page_url)">{{
					item.lang_name }}</el-button>
				<p>{{ tips }}
					<el-button @click="redistribution()" v-if="tips" type="primary">重新发布</el-button>
				</p>
			</el-row>
...
<script>
import siteLayout from './layouts/LayoutForGbAd.vue'
import {
	gbAdGetActivityList, gbAdAddActivity, gbAdUpdateActivity, gbAdDeleteActivity, gbAdBatchAddPage, gbAdBatchEditPage, getAdGoodsUrl, gbAdVerifyPage, gbAdVerifyActivity, gbAdConvertToMPage,
	gbAdGetAccessLink, gbAdGetPageList, gbAdLockingActivity, gbAdActReleased, gbAdGetMActivityList,

	addActivity, updateActivity, deleteActivity, verifyActivity, lockingActivity,
	gbAdRefreshSite, refreshSelete, batchEditPage,
	gbAdDeletePage,deletePage, verifyPage,
	getLangKeyList, gbAdGetPageTemplateList, gbAdGetAppActivityList,
	convertToAppPage, convertToMPage, getAccessLink,GB_getGbadChannelUrlList,
	actReleased, adActReleased, batchAddPage
} from '../plugin/api'
...
export default {
	...
	methods: {
		async getPageTemplates (scrollType) {
			...
			let res = await gbAdGetPageTemplateList(params)
		 ...
		 * 获取广告列表
		 */
		async getGbAdActivityList () {
			let activityTypeId = this.subpageId ? '2' : null;
			let params = {
		...
		async gbAdGetAppActivityList () {
			let res = await gbAdGetAppActivityList({})
		...
		async gbAdGetMActivityList () {
			let res = await gbAdGetMActivityList()
		...
		createActivity () {
			...
			this.activityForm.dialogTitle = '新增广告落地页'
		...
		createPage (activityId, langList, placeList, is_lock, create_user) {
			...
			if ((is_lock == 1 && create_user != this.siteInfo.userName) && this.siteInfo.isSuper != 1) {
				...
				/* 分享微数据 */
				// this.pageForm.shareTitle = ''
				// this.pageForm.shareDesc = ''
				// this.pageForm.shareImage = ''
		...
		async getPages (activityId) {
			let params = {
				...
				res = await gbAdGetPageList(params)
					...
					if (formName == 'activityForm') {
						...
						if (this.activityForm.id == '') {
							...
							if (this.activityForm.miss_count_status > 1) {
								...
								res = await gbAdAddActivity(platformData)
							} else {
								res = await gbAdAddActivity(platformData)
							}
						} else {
							params.id = this.activityForm.id
							res = await gbAdUpdateActivity(params)
						}
						...
						if (this.pageForm.id != '') {
							...
							res = await gbAdBatchEditPage(jsonObject)
						}
						// 新增
						else {
							let flag = true
							...
							res = await gbAdBatchAddPage(jsonObject)
						}
						...
						res = await gbAdConvertToMPage(params)
		...
		async verifyActivity (id, status, is_lock, create_user) {
			if ((is_lock == 1 && create_user != this.siteInfo.userName) && this.siteInfo.isSuper != 1) {
				...
				this.confirm(tip, async (vm) => {
					...
					await gbAdVerifyActivity(params)
					vm.isDetailActive = false
					vm.getGbAdActivityList()
					vm.expandRowKeys = []
				})
			}
		},
			...
			if ((is_lock == 1 && create_user != this.siteInfo.userName) && this.siteInfo.isSuper != 1) {
				...
				this.confirm('活动删除后，其所有的子页面也将被删除，确认删除？', async (vm) => {
					let params = {
						...
						res = await gbAdDeleteActivity(params)
			...
			if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
				...
				this.confirm('确认删除该页面?', async (vm) => {
					let params = {
						...
						res = await gbAdDeletePage(params)
		...
		async viewPages (id) {
			let params = {
				...
				res = await gbAdGetAccessLink(params)
			if (res.code == 0) {
				this.dialogLinksVisible = true
		...
		async redistribution () {
			let params = {
				...
				res = await gbAdActReleased(params)
		...
		async openCountryURL (id) {
			let res = await GB_getGbadChannelUrlList({ page_id: id });
		...
		handleTabClick () {
			...
			if (this.langList[0].key == 'en') {
				...
				if (lang != 'en') {
					...
					if (this.pageForm.data[lang].title == '') {
						this.pageForm.title = this.pageForm.data.en.title
						this.pageForm.data[lang].title = this.pageForm.data.en.title
					} else {
						this.pageForm.title = this.pageForm.data[lang].title//不同语言url
					}
			...
			// this.pageForm.redirect_url = this.pageForm.data[lang].redirect_url
			// this.pageForm.tplId = this.pageForm.data[lang].tplId
			...
			/* 分享同步 */
			// this.pageForm.shareTitle = this.pageForm.data[lang].shareTitle
			// this.pageForm.shareDesc = this.pageForm.data[lang].shareDesc
			// this.pageForm.shareImage = this.pageForm.data[lang].shareImage
		},
		getRowKey (row) {
			return row.id
		},
			...
			if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
				...
				this.confirm(tip, async (vm) => {
					let params = {
						...
						res = await gbAdVerifyPage(params)
		...
		gbAdLockingActivity (id, is_lock, create_user, $event) {
			...
			if ((create_user != this.siteInfo.userName) && this.siteInfo.isSuper != 1) {
				...
				this.$confirm(tip, '提示', {
					...
					let params = {
						...
						res = await gbAdLockingActivity(params)
		...
		updatePageShareField (data, name) {
			this.pageForm.data[this.currentLanguage][name] = data
		},
		...
		async getSupportLangs () {
			let res = await getLangKeyList({ 'activity_type': 3 })
...
/* 落地页活动 */
.geshop-Activity-btn-add {
  width: 160px;
}

/* 搜索活动名称 */
.geshop-index-btn-search {
  display: block;
  width: 200px;
  height: 40px;
  border-radius: 4px;
...
.geshop-index-btn-dosearch {
	position: absolute;
	right: 20px;
	top: 16px;
}
.geshop-index-btn-search .el-input__suffix {
  right: 0px;
  left: 8px;
  width: 32px;
}
.geshop-index-btn-search .el-input__inner {
  padding-right: 0px;
  padding-left: 40px;
}
.geshop-index-btn-search .geshop-icon-search-gray {
  margin-top: 10px;
}
</style>
