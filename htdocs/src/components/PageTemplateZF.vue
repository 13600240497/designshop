                ...
                <el-form :inline="true" :model="search">
                    ...
                    <el-form-item label="应用端口">
                        <el-select v-model="templateTabName" @change="handleTemplateSelect" placeholder="请选择">
                            <el-option label="全部" value="">全部</el-option>
                            <el-option v-for="(item, key) in places" :key="key" :label="item.platform_name" :name="key"
                                       :value="key"></el-option>
                        </el-select>
                    </el-form-item>
                    <!-- <el-form-item label="渠道">
                        <el-select v-model="search.pipeline" @change="handleFallSiteSelect" clearable placeholder="请选择">
                            <el-option label="全部" value=""></el-option>
                            <el-option v-for="(item, index) in allFallenSiteList" :key="index" :label="item.name" :name="item.code" :value="item.code"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="语种">
                        <el-select v-model="search.lang" clearable placeholder="请选择">
                            <el-option label="全部" value=""></el-option>
                            <el-option v-for="item in supportLangs" :key="item.code" :label="item.name" :value="item.code"></el-option>
                        </el-select>
                    </el-form-item> -->
                ...
                <el-table :data="templateList" style="width: 100%">
                    ...
                    <el-table-column prop="templateTabName" label="应用端口">
                        <template slot-scope="scope">
                            ...
                            <!-- <span v-if="scope.row.platform_name=='PC'">PC</span>
                            <span v-else-if="scope.row.platform_name=='M'">WAP</span>
                            <span v-else-if="scope.row.platform_name=='APP'">APP</span> -->
            ...
            <el-form :model="form" :rules="rules" ref="form" label-width="100px">
                ...
                <el-form-item label="应用环境">
                    <el-select v-model="form.place" placeholder="请选择" disabled>
                        <el-option label="活动页" value="1">活动页</el-option>
                        <el-option label="首页" value="2">首页</el-option>
                        <el-option label="首页" value="3">推广页</el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="模板类型">
                ...
                <!-- <el-form-item>
                    <el-upload action="/activity/page-tpl/upload-pic" name="files" accept="image/jpg,image/jpeg,image/png" :on-success="handleUploadSuccess"
                      :on-exceed="handleUploadExceed" :on-error="handleUploadError" :file-list="fileList" :before-upload="handleBeforeUpload">
...
import {
    ...
    ZF_updatePageTemplate,
    setDefaultPageTemplate,
    ZF_deletePageTemplate,
    getModelHtml,
    ...
    methods: {
        ...
        async getTemplates () {
            let params = {
                    ...
                    pageSize: 10,
                    // site_code: this.search.site_code
        ...
        async submitForm (formName) {
            ...
            this.$refs[formName].validate(async (valid) => {
                if (valid) {
                    ...
                    let res = await ZF_updatePageTemplate(params);
                    if (res.code == 0) {
                        this.resetForm('form');
                        this.getTemplates();
                    } else {
         ...
         * 选择渠道信息筛选出语言
         */
        handleFallSiteSelect (val) {
            this.search.pipeline = val;
            let supportLangs = [];
            this.allFallenSiteList.forEach(item => {
                if (val == item['code']) {
                    let lang_list = item['lang_list'];
                    Object.keys(lang_list).forEach(lang => {
                        supportLangs.push(lang_list[lang]);
                    });
                }
            });
            this.search.lang = '';
        ...
        deleteTemplate (id) {
            this.$confirm('确定要删除该页面模板', '提示', {
                ...
                let res = await ZF_deletePageTemplate(params);
        ...
        async seeTemplate (row) {
            ...
            let placeDomain = place == 1 ? '/activity' : '/home';
            this.viewModel.src = placeDomain + '/zf/page-tpl/preview?pid=' + pid + '&lang=' + langDefualt + '&id=' + id + '&site_code=' + site_code + '&pipeline=' + pipeline;

            let sideType = site_code.split('-')[1], sideWidth;
            if (sideType != 'pc' && sideType != 'web') {
                sideWidth = '400px';
            } else {
                sideWidth = '80%';
            }
        ...
        async setDefaultTemplate (id) {
            let res = await setDefaultPageTemplate({
                id: id
            });
