...
import { getUiComponentList, getPageUiTplList, deletePageUiTplList } from '../plugin/api'
	...
	methods: {
		...
		async getPageUiTemplateList () {
			let params = {
				...
				res = await getPageUiTplList(params)
		...
		async getComponentList (place) {
			let params = { 
				...
				res = await getUiComponentList(params)
			if (res.code == 0) {
				// 如果有传place字段（应用环境）
				if (place) {
		...
		deleteTemplate (id) {
			this.$confirm('确定要删除该页面模板？', '提示', {
				...
				let res = await deletePageUiTplList(params)
