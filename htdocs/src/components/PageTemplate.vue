...
import { getLangKeyList, getPageTemplateList, updatePageTemplate, setDefaultPageTemplate, deletePageTemplate, getModelHtml } from '../plugin/api'
	...
	methods: {
		async getTemplates () {
			let params = {
				...
				res = await getPageTemplateList(params)
		...
		async submitForm (formName) {
			...
			this.$refs[formName].validate(async (valid) => {
				if (valid) {
					...
					let res = await updatePageTemplate(params)
					if (res.code == 0) {
						this.resetForm('form')
						this.getTemplates()
					} else {
		...
		deleteTemplate (id) {
			this.$confirm('确定要删除该页面模板', '提示', {
				...
				let res = await deletePageTemplate(params)
