										...
										<div class="child-pages-container">
											...
											<el-tooltip content="转APP端" placement="bottom" effect="light">
												<el-button class="icon-geshop-mobile" @click="convertAPP(list.id, list.is_lock, list.activity_create_user, list.group_info, list.group_languages, scope.row.pipelineList, list.group_id, list.activity_id)"
														   v-if="list.activity_type == 7 && list.is_native == 0"></el-button>
											</el-tooltip>
...
<script>
	...
	function getPlatforms (data) {
		let obj = {};
		try {
			for( let i of GESHOP_PLOATFORMS) {
				obj[i] = data;
			}
		} catch (e) {
			console.log(e);
		}
		return clone_simple(obj);
	...
	export default {
		...
		data() {
			return {
				...
				// pc,m,app
				activityTabName: GESHOP_PLOATFORMS[0],
				places: [],
				editPlaces: [],
				pc_status: true,
				m_status: false,
				app_status: false,
				pageFormPlace: [GESHOP_PLOATFORMS[0]],
				...
				activityForm: {
					...
					// 应用渠道
					channelValue: getPlatforms([]),
					originChannelValue: getPlatforms([]),
					channelValueLength: getPlatforms(0),
					currentPlaceTabName: GESHOP_PLOATFORMS[0],
					currentSelectedLanguages: [],
					syncChannelInfo: getPlatforms({}),
					syncChannelValue: getPlatforms(''),
					place_channel_list: {},
					syncChannelStatus: getPlatforms(0),
					fallenValue: [],
					editfallenValue: [],
					...
					// 新增活动选中数据
					all_pipe_selected: getPlatforms({'currentChannel':'', 'check_all_pipe': true, 'check_indeterminate': false, 'pipeList':{}}),
					// 编辑渠道初始值
					all_pipe_original:getPlatforms({'currentChannel':'', 'check_all_pipe': true, 'check_indeterminate': false, 'pipeList':{}}),
					// 编辑同步信息
					all_pipe_edit_info: getPlatforms({ 'pipe_arr':[], 'pipe_selected':{}, 'sync_show': false }),
					all_pipe_status: 'add'
				},
				activityRules: {
					type: [{
				...
				timer:null,
				sub_page_edit_version: 1 // 子页面编辑的版本
			}
		},
		computed: {
			firstChannel() {
		...
		methods: {
			...
			handleFilterChannelList () {
				let	place_channel_list = getPlatforms([])
			...
			createActivity() {
				...
				let pc_supportPipelines = supportPipelines.web,
						wap_supportPipelines = supportPipelines.wap,
						app_supportPipelines = supportPipelines.app

				let first_pc_pipeline = pc_supportPipelines && pc_supportPipelines[Object.keys(pc_supportPipelines)[0]],
						first_wap_pipeline = wap_supportPipelines && wap_supportPipelines[Object.keys(wap_supportPipelines)[0]],
			...
			handleAllPipe(){
				...
				// console.log(currentPlaceTabName, this.activityForm.all_pipe_selected['app'], this.activityForm.all_pipe_selected['web']);
			},
			...
			editActivity(row, is_lock, create_user) {
				if ((is_lock == 1 && create_user != this.siteInfo.userName) && this.siteInfo.isSuper != 1) {
					...
					this.isShowAllCheckbox = getPlatforms(false)

					this.channelCheckboxcheckAll = getPlatforms(false)
					...
					// 初始化“默认同步信息”模板
					for(let i of GESHOP_PLOATFORMS) {
						this.activityForm.syncChannelStatus[i] = 0;
					}


					// 端下的渠道信息
					let support_list = row.group_info.support_list,
							pc_channel_value = support_list[GESHOP_PLOATFORMS[0]] && Object.keys(support_list[GESHOP_PLOATFORMS[0]]),
					...
					if (typeof pc_channel_value === 'undefined') {
						this.activityForm.currentDisabledPlatform.push(GESHOP_PLOATFORMS[0])
					}
					...
					// 初始化端口下渠道信息
					this.activityForm.channelValue[GESHOP_PLOATFORMS[0]] = pc_channel_value
					this.activityForm.channelValue.wap = wap_channel_value
					this.activityForm.channelValue.app = app_channel_value
					this.activityForm.channelValueLength[GESHOP_PLOATFORMS[0]] = pc_channel_value && pc_channel_value.length
					...
					// 编辑时渠道值（新增活动时提交的数据）
					this.activityForm.originChannelValue[GESHOP_PLOATFORMS[0]] = pc_channel_value
					...
					this.activityForm.syncChannelValue[GESHOP_PLOATFORMS[0]] = pc_channel_value && pc_channel_value[0]
					...
					if (default_platam == GESHOP_PLOATFORMS[0]) {
			...
			createPage(activityId, langList, placeList, is_lock, create_user, pipeline_list, row) {
				...
				if ((is_lock == 1 && create_user != this.siteInfo.userName) && this.siteInfo.isSuper != 1) {
					...
					if (pagePlacesArr.indexOf(GESHOP_PLOATFORMS[0]) != -1) {
					...
					if (platformKeys.indexOf(GESHOP_PLOATFORMS[0]) != -1 && platformKeys.indexOf('wap') == -1) {
			...
			editPage(row, is_lock, activity_create_user, langList, placeList, pipelineObject) {
				this.sub_page_edit_version = row.version || 1;
				let _this = this
				...
				this.$refs[formName].validate(async (valid) => {
					if (valid) {
						...
						if (formName == 'activityForm') {
							...
							let channelValue = JSON.parse(JSON.stringify(this.activityForm.channelValue));
							// key - pc, wap, app
							GESHOP_PLOATFORMS.forEach((key) => {
                            ...
                            place_channel_obj_list[key].forEach((item) => {
								...
								// console.log(support_channel_obj_list, key);
								let place_channel_object = {}
								// item - { code: 'ZF', lang_list: {...}, name: '全球站' }
						...
						// 新增子页面
							...
							this.$refs['publicPageForm'].validate(async (valid) => {
								...
								this.$refs['pagePipeRules'].validate(async (valid) => {
									...
									this.channelList.forEach((item) => {
										...
										Object.keys(langList).forEach((lang) => {
											...
											this.pagePlaces.forEach((it) => {
												let k = it['code'] // { code: GESHOP_PLOATFORMS[0], name: GESHOP_PLOATFORMS[0] }
									...
									jsonObject['version'] = this.sub_page_edit_version || 1;
						...
						// M转APP
							...
							if (this.convertForm.is_group == 1) {
								...
								source[source_channelCurrent] = {
									id: source_group_pipelineList ? source_group_pipelineList[source_channelCurrent]['page_id'] : '',
			...
			convertAPP(id, is_lock, activity_create_user, group_info, group_languages, pipelineList, group_id, activity_id) {
				if (is_lock == 1 && activity_create_user != this.siteInfo.userName && this.siteInfo.isSuper != 1) {
                    ...
                    // web端区分platform_list web
                    let source_group_platform_list = group_info.platform_list.web ? group_info.platform_list.web : ''
			...
			handleModelTempSelect(val) {
				...
				console.log(GESHOP_PLOATFORMS[0]);
				// PC、M、APP页面模板数据获取
				if (this.templateSelectPlace == GESHOP_PLOATFORMS[0]) {
					this.getTmpListValue = GESHOP_PLOATFORMS[0]
			...
			handleSureModel() {
				...
				if (currentPlace == GESHOP_PLOATFORMS[0]) {
					...
					timer = setTimeout(function () {
						if (panelCont0.clientHeight + panelCont0.scrollTop == panelCont0.scrollHeight) {
							...
							if (tempNum <= _this.tplInfo.maxPageNo) {
								...
								if (_this.templateSelectPlace == GESHOP_PLOATFORMS[0]) {
									_this.getTmpListValue = GESHOP_PLOATFORMS[0]
			...
			async seeTemplate(pid, lang, id, site_code) {
				...
				let sideType = site_code.split('-')[1],
						sideWidth
				if (sideType != GESHOP_PLOATFORMS[0]) {
					sideWidth = '400px'
				} else {
					sideWidth = '100%'
				}
			...
			handle_show_updateHead () {
				const platform = this.activityTabName || GESHOP_PLOATFORMS[0];
