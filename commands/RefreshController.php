...
class RefreshController extends Controller
{
    ...
    const DEBUG_TASK_NUM   = 16; //本地和测试环境
    ...
    public function actionStart()
    {
        ...
        $this->serv->set(
            [
                ...
                'daemonize'                => true,
                'log_file'                 => dirname(__DIR__) . '/runtime/logs/refresh_swoole.log',
                //'log_level'             => SWOOLE_LOG_ERROR,
                'pid_file'                 => '/tmp/swooleTaskDebug.pid',
                'max_request'              => 100,
    ...
    {
        echo "Start\n";
        cli_set_process_title("refresh_task_master");
    }
    ...
    public function onReceive(\swoole_server $serv, $fd, $from_id, $data)
    {
        ...
        if (!is_array($body) || empty($body['data']) || empty($body['log_id'])) {
        ...
        $action = $body['action'];
        $tasks = array_map(function ($item) use ($action) {
            return ['action' => $action, 'result' => $item];
        }, $body['data']);
        $tasks = array_chunk($tasks, 25);

        foreach ($tasks as $task) {
            sleep(1);
            $results = $serv->taskWaitMulti($task, $this->timeOut);
            $this->workerClass->recordRefreshPageLog($results, $body['log_id']);
        }
    }
    ...
    public function onTask($serv, $task_id, $from_id, $data)
    {
        ...
        if (method_exists($this->workerClass, $data['action'])) {
            return $this->workerClass->{$data['action']}($data['result']);
        }
    }
